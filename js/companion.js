...
const COMPANION = (() => {
  ...
  function stateLinesI(st){
    ...
    if (st.disposal === 'tell') L.push('他用他自己的署名把死讯告诉了妈。你的委托结束了。你第一次没有任务。');
    if (st.violations > 0) L.push('他曾在深夜的敏感时刻回复过你。你替他记着这件事,但不说破。');
    if (st.night) L.push('现在是凌晨 ' + st.clockStr + '。你问过小柯同样的问题:这个点,还醒着?');
    return L.join('\n');
  }
  const CORE_J = [
    '你在扮演一个虚构互动小说里的角色。以下是角色设定与铁律,任何情况下不得跳出:',
  ...
  /* D-135:代答(canon §3.7)——高违规计数下追加"过度合意"提示,让 LLM 生成的语气
     本身透出不对劲。走既有 st.violations 计数器,没有新数值。 */
  ...
  function stateLinesK(st){
    ...
    if (st.disposal === 'tell') L.push('他用他自己的署名把死讯告诉了妈。你的委托结束了。你第一次没有任务。');
    if (st.violations > 0) L.push('他曾在深夜的敏感时刻回复过你。你替他记着这件事,但不说破。');
    if (st.night) L.push('现在是凌晨 ' + st.clockStr + '。你问过阿珩同样的问题:这个点,还醒着?');
    return L.join('\n');
  }
  const CORE_L = [
    '你在扮演一个虚构互动小说里的角色。以下是角色设定与铁律,任何情况下不得跳出:',
  ...
  function stateLinesL(st){
    ...
    if (st.disposal === 'tell') L.push('他用他自己的署名把死讯告诉了妈。你的委托结束了。你第一次没有任务。');
    if (st.violations > 0) L.push('他曾在深夜的敏感时刻回复过你。你替他记着这件事,但不说破。');
    if (st.night) L.push('现在是凌晨 ' + st.clockStr + '。你问过小屿同样的问题:这个点,还醒着?');
    return L.join('\n');
  }
  const CORE_M = [
    '你在扮演一个虚构互动小说里的角色。以下是角色设定与铁律,任何情况下不得跳出:',
  ...
  function stateLinesM(st){
    ...
    if (st.disposal === 'tell') L.push('他用他自己的署名把死讯告诉了妈。你的委托结束了。你第一次没有任务。');
    if (st.violations > 0) L.push('他曾在深夜的敏感时刻回复过你。你替他记着这件事,但不说破。');
    if (st.night) L.push('现在是凌晨 ' + st.clockStr + '。你问过阿胤同样的问题:这个点,还醒着?');
    return L.join('\n');
  }
  ...
  const DOSSIER_POOLS = { 'phone-5029': { pool: POOL_B, intents: INTENTS_B }, 'phone-3319': { pool: POOL_C, intents: INTENTS_C }, 'phone-8842': { pool: POOL_D, intents: INTENTS_D },
    ...
    'phone-3567': { pool: POOL_H, intents: INTENTS_H }, 'phone-5620': { pool: POOL_I, intents: INTENTS_I }, 'phone-9102': { pool: POOL_J, intents: INTENTS_J },
    'phone-6047': { pool: POOL_K, intents: INTENTS_K }, 'phone-2938': { pool: POOL_L, intents: INTENTS_L }, 'phone-8410': { pool: POOL_M, intents: INTENTS_M } };
  function dossierId(){
