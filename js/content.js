...
const CONTENT = (() => {
  ...
  const DOSSIER = {
    ...
    cast: {
      companion: { key: 'rou', label: '柔柔 ♥' },
      mother:    { key: 'mom', label: '妈' },
      contacts:  [ { key: 'zhou', label: '老周', lastContact: '47天前' } ],
    },
    /* ==================== D-109 第②步:证据链 E1–E5 + 真相文案 ====================
       schema §2.4。`source` 是给 P2 幻觉零容忍门禁用的定位标签(哪条深搜给出这条
       证据),这一步只搬"名字/文案"这类干净数据表,**不**重接 `evidence('E2')` 这类
       触发点——那些是嵌在各屏幕控制流里的机制接线,搬了没有即时收益(同一份底本),
       留在原处。`truth` 两页是案卷合拢前的真相揭示文案,原样搬入。 */
    evidence: {
    ...
    /* ==================== D-109 第③步:threads(收件箱/菜单线程清单) ====================
       schema §2.3。只搬"哪些线程 + 叫什么名字 + 点了去哪"这类静态表;未读徽标(哪条读过/
       删除态覆盖标签/窗口收件人)是活状态驱动的框架逻辑,原样留在 `inboxItems()` 里,只是
       现在从这张表读 id/label/to,不再写死整个 rows 数组。`bottleIn` 标 `scenario:'B'`——
       B 局专属收件人条目,呼应 `meta.scenarios`(拍板⑤,B 是同一底本的场景分支)。 */
    threads: {
      menu: [
        { label: '1 消息',        to: 'inbox' },
    ...
    /* ==================== D-109 第⑤步b:encounters/endings ====================
       schema §2.7。只搬已经是纯数据(无嵌入函数)的两张表:
       - `encounters.huntGraze`:回收进程"擦过"的受限裁决叙述集(D-105),纯 id+文本,
         数值恒定、机制在 engine 侧,原样搬入。
       - `endings.disposal`:处置(继续/删除/告知)在结局屏的回声文案,纯字符串表。
       **范围校准(与①②③④同一纪律)**:`SCREENS.dilemma` 的两难池(D-106)本轮**不**
       抽——它每条选项直接嵌了一个执行 `ENGINE.act(...)` 的闭包(不是纯数据),要把
       它做成"数据"需要先设计一套动作效果的小 DSL(内容层描述"扣什么/给什么/说什么"、
       框架层解释执行),这是一次真正的架构改动,不是"结构搬家"——留到那件事单独立项
       再做,现在硬搬只会把闭包塞进 JSON 不像的东西里,不是抽象。 */
    encounters: {
      huntGraze: [
