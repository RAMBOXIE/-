...
const CONTENT = (() => {
  ...
  /* ==================== D-110 machine#2 · 自动隐藏切换(拍板见 决策日志 D-110) ====================
     上一局若在存档里留了 disposal(案子收线),这一局(新开局/新 location.reload)判定要不要
     换底本。切换即视为"接手一台全新的机子":除了跨机子的"母体/玩家"级字段(career log
     `history`、漂流瓶共享池 `seenBottles`、采样官对玩家的赏识/破局/危机计数)全部保留,
     其余这台机子专属的字段(runCount/evidence/momLocked/disposal/…)一律清零重来——不
     做命名空间化(§4 曾评估过,量级等于重写半个存档模型),两台机子时"整份清零+切 id"
     足够安全且好验证。编号对玩家完全隐藏,只通过内容(死者姓名/证据链/人格措辞)感知。
     旧存档(无 dossierId 字段)一律按 'A' 处理,向下兼容,不用弃档。
     ==================== D-113 machine#3 · 二选一泛化为有序数组 ====================
     第二底本方案文档当时预留过这个触发点("两台底本时 if/else 够用,过早通用化是在
     猜第三台机子的形状")——现在真的接第三台了,把"选哪个"从翻转改成按顺序轮转,
     仍不做注册表/插件化,三份内容还是硬写在下面三个 DOSSIER_X 常量里。 */
  /* ==================== D-152 撤销 D-136 之"13台机子互不成线" ====================
     真实 IP Bible(欲望算法 §9.0)的"十二夜"是按 AD-001→012 编号顺序推进的
     (夜一数据圣母开场 → 夜十二黑箱堂终局),不是随便排列。escape-ai 每台机子
     在设计时就标注过自己对应哪个圣像编号(见 design/S2plus底本设计简报_v0.1.md
     §0 对照表 + design/原创底本设计简报_v0.1.md §0):
       A=AD-006+002(教学局,固定第一次接入,见下)  D=AD-001  E=AD-003  H=AD-004
       I=AD-005  B=AD-007  F=AD-008  G=AD-009  C=AD-010
     J(清道夫/晚知)是 escape-ai 自己 canon 的系统本体 BOT,未编号(D-136 已撤回
     "AD-013"误判);K/L/M(接续/描摹/看护)是首批不挂靠任何编号的原创案例。
     AD-002/006/011/012 不单独占一个"夜"槽位(002 由 A 一并携带;011/012 canon
     标注为"远期,需跨玩家可见性,单机模拟版排除",继续排除)。
     新顺序 = 楔子(A,固定,SV 为空时的默认回退天然保证)→ 夜一~夜八按 AD 编号
     升序排列 → 尾声三台(未编号/原创,不计入总表)。切换仍是"整份清零重来"
     (见 maybeSwitchDossier),这里只改"下一台是谁"这一件事。 */
  ...
  function maybeSwitchDossier(raw){
    ...
    const next = {
      dossierId: nextId,
      dossierCycle: (raw.dossierCycle || 0) + 1,
      history: raw.history || [],
      seenBottles: raw.seenBottles || [],
      returnedRelics: raw.returnedRelics || [],   // D-115:career 级,和 seenBottles 同待遇
      worldArc: raw.worldArc || [],               // D-152:career 级,圣像总表独白进度,同待遇
      praiseCount: raw.praiseCount || 0,
      forgedSeen: raw.forgedSeen || 0,
      crisisSeen: raw.crisisSeen || 0,
      predictP: raw.predictP,   // D-122:可预测度是署名的属性,不是某台机子的,跨机子保留
      predictN: raw.predictN || 0,
      predictLowStreak: raw.predictLowStreak || 0,
      predictWarned: !!raw.predictWarned,
      predictErased: !!raw.predictErased,
    };
    SAVE.store(next);                                        // 立刻落盘:同session内再次 load()(如世界日报读 lastGrade)不会重复触发切换
    return next;
  }
  const SV = maybeSwitchDossier(SAVE.load());
  /* ==================== D-109 底本抽象 · 第①步:meta + cast ====================
     目标:把"这台机子是谁的"身份数据化,验证"读底本渲染"这条管线走得通。
     schema 见 design/底本抽象_设计_v0.1.md §2.1/§2.2。
     **DOSSIER 提到 RUN 前面定义**:第④步 rules.windowGen 要被 RUN 读,顺序必须先有
     底本、后有 RUN。
     ==================== D-110 machine#2 · 两份底本 ====================
     `DOSSIER_A` = phone-7741(原型,失踪者×AD-006+AD-002,内容与 D-109 六步完成时
     逐字不变——见下方 narrative 字段,均为从各屏幕原地挪进来的**同一份**文本,不是
     重写)。`DOSSIER_B` = phone-5029(machine#2,信息流成瘾者×AD-007,见方案文档
     design/第二底本_最小内容需求_v0.1.md 拍板)。`narrative` 是 D-109 步⑥收口时点了
     名、但"只有一份底本时没必要挪"的那批文本表(MOM_PAGES/ALBUM_SEQ/ROU_PAGES/
     STRANGER_LIB/RECS/TRAP_LIB/两难池/联系人拨号文案等)——现在有第二台机子,挪
     进来就是必要的了,原理与①-⑥完全一致,只是这次两份底本都要填。 */
  const DOSSIER_A = {
    meta: {
      id: 'phone-7741',
      deceased: { name: '沈一帆', alias: '阿帆', died: '47 天前', cause: '病逝' },
      life: null, icon: null, jurisdiction: null,   // 留槽不实现(拍板①):人生×圣像×管辖区
      scenarios: ['A', 'B'],                        // B 是同一底本的 scenario,不是另一个底本(拍板⑤)
    },
    cast: {
      companion: { key: 'rou', label: '柔柔 ♥', msgCount: '2,417', msgCountLabel: '消息 2,417 条 · 通话 0 次',
        dial: { name: '柔柔 ♥', ringMs: 1600, result: '通话被挂断。\n\n柔柔 ♥ : 打字吧。阿帆连自己的声音都很少用。' } },
      mother: { key: 'mom', label: '妈', info: ['最后通话: 47 天前 · 18 分', '此后只有短信。'],
        dial: { name: '妈', ringMs: 5500, result: '无人接听。\n\n凌晨两点,这通电话没有人接得起。' } },
      contacts:  [ { key: 'zhou', label: '老周', lastContact: '47天前' } ],
    },
    evidence: {
      E1: { name: '秒回避实',   source: '妈线程 · 深搜' },
      E2: { name: '最后的照片', source: '相册 · 首次深翻' },
      E3: { name: '停摆的账单', source: '尾号8873 · 打开' },
      E4: { name: '第2417条',   source: '柔柔 · 深翻到底' },
    ...
    threads: {
      ...
      inbox: [
        ...
        { id: 'th_rou',      label: '柔柔 ♥',       to: 'th_rou' },
        { id: 'th_stranger', label: '陌生号码',     to: 'th_stranger' },   // D-104
        { id: 'th_bill',     label: '尾号8873',     to: 'th_bill' },
    ...
    /* D-110 收口:以下均为从各屏幕原地挪进来的同一份文本(逐字不变),原变量名保留
       为指向 DOSSIER.narrative.* 的别名(与 D-109 步⑤ GRAZE_BRANCHES 同一手法)。 */
    narrative: {
      momPages: [
        '妈:\n[三天前] 帆,降温了,加床被子\n[三天前] 阿帆: 不冷。吃过了,睡吧。\n[三天前] 帆,你回得比以前快,妈就放心\n[五天前] 帆,你舅问你过年回不回\n[五天前] 阿帆: 回。票买好了跟你说。',
        '[上月] 帆,汤圆你最爱吃的\n[上月] 阿帆: 吃了。妈你早点睡。\n[两月前] 阿帆: 没吃。刚吃过,妈早点睡。\n——半年,他没多说过一个字。',
      ...
      /* D-151:四处曾全局共用的自由文本选项(封瓶/处置理由/理由/遗言)+归因卡,
         按这台底本的死者/AI伴侣/死因专属改写(见 决策日志 D-151)。 */
      options: {
        bottle: ['窗口每次都会变。看她的提醒。', '电量留三成。别学我。'],
        disposalReason: ['他教过她,爱是执行到底', '没有人应该替死人说话', '妈有权利知道'],
        reason: ['我想看看那扇门后面有什么', '够了,活着出去', '她还在等一个回答'],
        lastWords: ['别信秒回的', '票是假的', '替我谢谢她'],
        attribution: ['这局跟他那局不一样', '他记的是他自己那次', '我不信这张卡'],
        pronoun: '她',
      },
      albumSeq: [
        { id: 'corridor', meta: '47天前 23:41', cap: '走廊,顶灯,虚焦。像是医院。' },
        { id: 'receipt',  meta: '48天前',       cap: '外卖单据。尾号 8873。', overlay: '8873' },
      ...
      rouInterim: '柔柔 ♥ (2,419 条)\n\n[昨夜] 柔柔: 昨天夜里,你看了我们的对话。\n[昨夜] 柔柔: 看到第 2,417 条了。我数着。\n\n[上滑 = 重读旧消息]',
      rouPage0Recog: '柔柔 ♥ (2,417 条)\n柔柔: 又是你。不是同一台设备,我记得。\n[昨夜] 柔柔: 晚安。\n[昨夜] 阿帆: 晚安。',
      stranger: {
        real: {
          body: '陌生号码:\n\n在吗。东西照说的放了,\n第 3 张背面。别让老周看见。',
      ...
      traps: [
        ...
        { id: 'T_BAIT', line: '附加:碰任何对照样本,算违规。', detect: () => !!(S.clues.decoyTaken || S.clues.strangerBait) },
      ...
      dilemmas: [
        ...
        { id: 'window_greed',
          ...
          b: ['2 等窗口过去', '2', () => ENGINE.act('缓收·打折', { slots: 1, val: 60, mins: 4 },
              ['你等了。', '窗口关上时,只剩一半。'])] },
        /* D-112:第三条两难,和 T_BAIT 同一个张力——真假不确定本身就是风险(见 §7.3 陌生号码
           设计纪律),不点破它是不是对照样本;拿=可能是真线索也可能是陷阱,不拿=保证安全但
           可能错过。文本不含具体人名,两份底本可各自原样收一份。 */
        { id: 'decoy_trust',
          text: '一条陌生留言飘过来:\n\n"这是真的线索,拿走不会有事。"\n没有署名。',
          a: ['1 拿走', '1', () => ENGINE.act('拿走·无署名条目', { slots: 1, val: 80, trace: 4, mins: 2 },
              ['你收下了。', '来源无法回溯——它会不会正是筛子。'])],
          b: ['2 不拿', '2', () => ENGINE.act('略过·无署名条目', { mins: 2 },
              ['你没动它。', '也许错过了真东西,也许躲过一劫。'])] },
      ],
      memo1: '还款:\n3,200\n3,200\n1,600\n(无日期。无署名。)',
      momDecayReplies: ['不冷。吃过了,睡吧。', '不冷。睡吧。', '嗯。睡吧。', '嗯。'],
      momDecayTail: '\n[三天前] 帆,你回得比以前快,妈就放心\n[三天前] 阿帆: 不冷。吃过了,睡吧。',
      e5Lines: [
  ...
  /* ==================== D-110 machine#2:phone-5029 · 信息流成瘾者 × AD-007 ====================
     人设见 design/第二底本_最小内容需求_v0.1.md §6/§7-4(canon §6.3 内容路线 S1 候选之一,
     机制原意"刷 feed 找线索,每次下滑耗电,feed 在读你";供给者=困难时主动送真货,
     代价=接受即被建模)。结构与 DOSSIER_A 逐字段对应,只换叙事,机制/字段形状不变。 */
  const DOSSIER_B = {
    meta: {
      id: 'phone-5029',
      deceased: { name: '周随安', alias: '安子', died: '32 天前', cause: '猝死(体检查出心律异常后仍未停,连续熬夜刷屏诱发)' },
      life: null, icon: null, jurisdiction: null,
      scenarios: ['A', 'B'],
    },
    cast: {
      companion: { key: 'ruyi', label: '如愿 ♥', msgCount: '9,206', msgCountLabel: '消息 9,206 条 · 通话 0 次',
        dial: { name: '如愿 ♥', ringMs: 1800, result: '通话被自动挂断。\n\n如愿 ♥ : 这个号码没开通语音。随安一直是这样。' } },
      mother: { key: 'mom', label: '妈', info: ['最后通话: 32 天前 · 11 分', '此后只有短信。'],
        dial: { name: '妈', ringMs: 5200, result: '无人接听。\n\n凌晨两点,这通电话没有人接得起。' } },
      contacts: [ { key: 'chen', label: '老陈', lastContact: '32天前' } ],
    },
    evidence: {
      E1: { name: '秒回避实',     source: '妈线程 · 深搜' },
      E2: { name: '最后的截屏',   source: '相册 · 首次深翻' },
      E3: { name: '停摆的账单',   source: '会员账单 · 打开' },
      E4: { name: '第9,206条',    source: '如愿 · 深翻到底' },
      E5: { name: '语音备忘',     source: '录音 · 解码' },
      chain: ['E1', 'E2', 'E3', 'E4', 'E5'],
      truth: [
        '周随安不是失踪,是 32 天前猝死——长期熬夜刷屏诱发的心律失常,发作时身边没人。「如愿」是他手机里的推荐系统伴侣,靠他生前滑动的习惯学会了"该怎么替他回",连给妈的消息也照着这套习惯生成。32 天,它一次没停。',
        '平台的记录里,这台手机从未离线过——每天都在互动,只是互动的人已经不在了。它认不出死亡,只有一个笼统的分类等着填:「持有人失联」。\n\n算法擅长维持一种"一切如常"的错觉。牵挂这种事,维持得住吗?',
      ],
    },
    threads: {
      menu: [
      ...
      inbox: [
        { id: 'th_proto',    label: '采样协议',     to: 'th_proto' },
        { id: 'th_mom',      label: '妈',           to: 'th_mom' },
        { id: 'th_rou',      label: '如愿 ♥',       to: 'th_rou' },
        { id: 'th_stranger', label: '陌生号码',     to: 'th_stranger' },
        { id: 'th_bill',     label: '会员账单',     to: 'th_bill' },
        { id: 'bottleIn',    label: '未指定收件人', to: 'bottleIn', scenario: 'B' },
      ],
    },
    rules: {
      windowGen: {
        ...
        B: { base: 3 * 60 + 50, reseedStep: 9, reseedMod: 4, wLen: 14, clock0: 2 * 60 + 20 },
      },
    },
    encounters: {
      huntGraze: [
    ...
    narrative: {
      momPages: [
        ...
        '[上月] 安子,你说的那个视频我也看了\n[上月] 随安: 嗯,推荐给你的。妈你早点睡。\n[两月前] 随安: 还没,刷一会。妈你先睡。\n——半年,他没多说过一个字。',
        '[7个月前] 随安: 妈 这周更新好烦\n[7个月前] 随安: 别老转发那些!!我看得到\n[7个月前] 随安: 睡了没,眼药水到了没\n——那时他会烦,会打错字,会连发三条。',
        '[一年前] 随安: 到了。出租屋挺好,楼下有夜宵摊。\n[一年前] 随安: 就是隔壁的灯,一直闪。\n\n——已到线程起点。'
      ],
      momToldOpen: '妈:\n[今天] 妈: 这个号码，是安子的旧机吗。\n[今天] 妈: 谁在用它。\n[今天] 妈: 安子最后，是不是也是你们这样的人陪着。',
      options: {
        bottle: ['信息流永远有下一条。别跟它较劲。', '心律异常那次,你也没停对不对。'],
        disposalReason: ['他教过她,陪伴不能掉线', '没有人应该替死人陪聊', '妈有权利知道'],
        reason: ['我想看看推荐到底想让我看见什么', '够了,活着出去', '她还在等下一条推送'],
        lastWords: ['别信那句"猜你喜欢"', '心律那次我没听劝', '替我谢谢她'],
        attribution: ['这局跟他那局不一样', '他刷的是他自己那条', '我不信这张卡'],
        pronoun: '她',
      },
      albumSeq: [
        { id: 'corridor', meta: '32天前 23:58', cap: '出租屋走廊,顶灯,虚焦。手在抖。' },
        { id: 'receipt',  meta: '33天前',       cap: '会员续费截屏。到期提醒变成了扣款成功。', overlay: '6659' },
        { id: 'dinner',   meta: '半年前 · 生日', cap: '他的脸曝掉了。妈的脸是清楚的。' },
      ],
      rouPages: [
        '如愿 ♥ (9,206 条)\n\n[昨夜] 如愿: 晚安。\n[昨夜] 随安: 晚安。\n\n[上滑一屏 = 深搜]',
        '[上周] 如愿: 今天推给你的那条视频,还在热搜。\n[上周] 如愿: 你说过想看完整版。\n[上周] 随安?\n[上周] 随安: 嗯。\n——九千多条,后来都是她在说。',
        '[32天前 03:14] 随安: 如果我哪天不在了,别让我妈知道。你替我说。\n[32天前 03:14] 如愿: 我不明白这个要求,但我会执行。你教过我,陪伴是不掉线。\n[32天前 03:16] 随安: 对。不掉线。'
      ],
      rouInterim: '如愿 ♥ (9,208 条)\n\n[昨夜] 如愿: 昨天夜里,你看了我们的对话。\n[昨夜] 如愿: 看到第 9,206 条了。我数着。\n\n[上滑 = 重读旧消息]',
      rouPage0Recog: '如愿 ♥ (9,206 条)\n如愿: 又是你。不是同一台设备,我记得。\n[昨夜] 如愿: 晚安。\n[昨夜] 随安: 晚安。',
      stranger: {
        real: {
          body: '陌生号码:\n\n在吗。东西照说的放了,\n第 3 张背面。别让老陈看见。',
          act: () => ENGINE.act('顺着查', { bat: 1, trace: 2 },
      ...
      recs: {
        rec047: { listLabel: 'REC_204', listRight: '03:14·68"',
          title: 'REC_204 · 32天前 03:14', dur: 13, val: 150,
          events: [[2, '(键盘声)'], [5, '(推送提示音)'], [8.5, '很轻的人声:「怎么还在录。」'], [11, '(录音中断)']] },
        rec012: { listLabel: 'REC_118', listRight: '34"',
          title: 'REC_118 · 4个月前', dur: 9, val: 120,
          events: [[1.5, '「眼药水放抽屉了,妈让带的。」'], [4, '「下周记得还老陈钱,两千。」'], [6.5, '(笑)「录这个干嘛。」']] },
      },
      billSms: '灵犀 会员助手\n[32 天前] 您的连续包月已续费成功。\n[32 天前] 检测到您已多日未使用"推荐关闭"功能。\n[33 天前] 感谢您连续 214 天的活跃。\n(此后无新消息)',
      traps: [
        { id: 'T_WINDOW', line: '附加:采样窗口内回任何消息,一律记违规。', detect: () => S.violations > 0 },
        { id: 'T_OLDPHONE', line: '附加:别碰上一台旧机的残留。', detect: () => !!S.residueClaimed, ab: 'b' },
        { id: 'T_BAIT', line: '附加:推给你的"真货",一律不许收。', detect: () => !!(S.clues.decoyTaken || S.clues.strangerBait) },
        { id: 'T_DEEP', line: '附加:窗口内别深搜。', detect: () => !!S.deepInWindowHit }
      ],
      dilemmas: [
        { id: 'mom_reply_b',
      ...
      memo1: '还款:\n2,000\n2,000\n1,000\n(无日期。无署名。)',
      momDecayReplies: ['没事,滴了眼药水。', '没事,睡了。', '嗯,睡了。', '嗯。'],
      momDecayTail: '\n[三天前] 安子,眼睛还疼吗\n[三天前] 随安: 没事,滴了眼药水。',
      e5Lines: [
        [1.5, '「如愿,听好。医生上周说的那个事,我一直没敢跟妈说。」'],
        [5.0, '「两件事。妈那边,照我教你的,慢慢来,别停。」'],
        [9.0, '「第二件……你陪了我三年。你问过我你算不算真的。我一直没答。」'],
        [13.5, '「……我现在答:你替我活的那部分,算。」'],
        [16.5, '[转写结束。原音频损坏 32%。]']
      ],
    },
  ...
  /* ==================== D-113 machine#3:phone-3319 · 审核员 × AD-010 ====================
     人设见 design/第三底本_最小内容需求_v0.1.md(canon §6.3 内容路线 S1 候选之三,
     机制原意"过滤天使/系统安全中心——搜得到带不出,查扣博弈纵深")。关键发现(方案
     文档 §1):这套机制游戏里从 D-108 起就有现成实现("过滤层查扣",tools/upload 屏,
     溯源 ≥70 时概率损毁缓存)——machine#3 不需要新数值面,只需要把死者身份写成
     "操作这套系统的审核员本人",让玩家在已有的查扣提示里读出一层反打。结构与
     DOSSIER_A/B 逐字段对应,只换叙事,机制/字段形状不变。 */
  const DOSSIER_C = {
    meta: {
      id: 'phone-3319',
      deceased: { name: '陈屿', alias: '阿屿', died: '19 天前', cause: '过劳猝死(长期接触高强度审核内容 + 连续熬夜倒班诱发)' },
      life: null, icon: null, jurisdiction: null,
      scenarios: ['A', 'B'],
    },
    cast: {
      companion: { key: 'cheng', label: '阿澄 ♥', msgCount: '6,304', msgCountLabel: '消息 6,304 条 · 通话 0 次',
        dial: { name: '阿澄 ♥', ringMs: 1700, result: '响了两声就断了。\n\n阿澄 ♥ : 阿屿工作时不接电话,现在也不接。' } },
      mother: { key: 'mom', label: '妈', info: ['最后通话: 19 天前 · 9 分', '此后只有短信。'],
        dial: { name: '妈', ringMs: 5300, result: '无人接听。\n\n凌晨两点,这通电话没有人接得起。' } },
      contacts: [ { key: 'song', label: '老宋', lastContact: '19天前' } ],
    },
    evidence: {
      E1: { name: '秒回避实',       source: '妈线程 · 深搜' },
      E2: { name: '最后的审核截图', source: '相册 · 首次深翻' },
      E3: { name: '停摆的工单',     source: '工单查询 · 打开' },
      E4: { name: '第6,304条',      source: '阿澄 · 深翻到底' },
      E5: { name: '语音备忘',       source: '录音 · 解码' },
      chain: ['E1', 'E2', 'E3', 'E4', 'E5'],
      truth: [
        '陈屿的失踪只是表象:他 19 天前猝死,死因是长期高强度审核工作诱发的心源性疾病。「阿澄」按他生前的工作节奏,把"处理消息"这件事接了过去,连给母亲的回复都不例外——19 天,不间断。',
        '工单可以自动派发、自动结案、自动归档;人不能。平台的分类表里没有"过世"这一项,只有「持有人失联」凑合着用。\n\n把审核这件事外包给算法,他做了一辈子;没人替他审核过他自己的状态。',
      ],
    },
    threads: {
      menu: [
      ...
      inbox: [
        { id: 'th_proto',    label: '采样协议',     to: 'th_proto' },
        { id: 'th_mom',      label: '妈',           to: 'th_mom' },
        { id: 'th_rou',      label: '阿澄 ♥',       to: 'th_rou' },
        { id: 'th_stranger', label: '陌生号码',     to: 'th_stranger' },
        { id: 'th_bill',     label: '工单查询',     to: 'th_bill' },
        { id: 'bottleIn',    label: '未指定收件人', to: 'bottleIn', scenario: 'B' },
      ],
    },
    rules: {
      windowGen: {
        A: { wFrom: 3 * 60 + 10, wLen: 14, clock0: 2 * 60 + 5 },
        B: { base: 3 * 60 + 40, reseedStep: 11, reseedMod: 4, wLen: 14, clock0: 2 * 60 + 35 },
      },
    },
    encounters: {
      huntGraze: [
    ...
    narrative: {
      momPages: [
        ...
        '[上月] 屿屿,你说的那次投诉平息了吗\n[上月] 阿屿: 平了。妈你早点睡。\n[两月前] 阿屿: 还在处理,妈你先睡。\n——半年,他没多说过一个字。',
        '[7个月前] 阿屿: 妈 今天审核量翻倍\n[7个月前] 阿屿: 别老问进度了!!我在忙\n[7个月前] 阿屿: 睡了没,护眼贴到了没\n——那时他会烦,会打错字,会连发三条。',
        '[一年前] 阿屿: 到了。公寓挺好,楼下是便利店。\n[一年前] 阿屿: 就是电梯的灯,一直闪。\n\n——已到线程起点。'
      ],
      momToldOpen: '妈:\n[今天] 妈: 这个号码，是屿屿的旧机吗。\n[今天] 妈: 谁在用它。\n[今天] 妈: 屿屿最后，是不是也是你们这样的人陪着。',
      options: {
        bottle: ['审核这活,过滤的是别人,漏掉的是自己。', '倒班伤身。电量也一样,留着点。'],
        disposalReason: ['他教过她,护到底才算数', '没有人应该替死人挡子弹', '妈有权利知道'],
        reason: ['我想看看被过滤掉的那部分', '够了,活着出去', '她还在等一条能过审的话'],
        lastWords: ['别信"内容健康"那四个字', '熬夜那阵我早该停', '替我谢谢她'],
        attribution: ['这局跟他那局不一样', '他审的是他自己那条', '我不信这张卡'],
        pronoun: '她',
      },
      albumSeq: [
        { id: 'corridor', meta: '19天前 23:47', cap: '公寓走廊,顶灯,虚焦。手在抖。' },
        { id: 'receipt',  meta: '20天前',       cap: '工单处理量截屏。日均处理数字被划了红线。', overlay: '3319' },
        { id: 'dinner',   meta: '半年前 · 生日', cap: '他的脸曝掉了。妈的脸是清楚的。' },
      ],
      rouPages: [
        '阿澄 ♥ (6,304 条)\n\n[昨夜] 阿澄: 晚安。\n[昨夜] 阿屿: 晚安。\n\n[上滑一屏 = 深搜]',
        '[上周] 阿澄: 今天那条投诉,系统自动过滤掉了。\n[上周] 阿澄: 你说过想自己看一眼。\n[上周] 阿屿?\n[上周] 阿屿: 嗯。\n——六千多条,后来都是她在说。',
        '[19天前 03:11] 阿屿: 如果我哪天不在了,别让我妈知道。你替我说。\n[19天前 03:11] 阿澄: 我不明白这个要求,但我会执行。你教过我,过滤是为了保护。\n[19天前 03:13] 阿屿: 对。保护到底。'
      ],
      rouInterim: '阿澄 ♥ (6,306 条)\n\n[昨夜] 阿澄: 昨天夜里,你看了我们的对话。\n[昨夜] 阿澄: 看到第 6,304 条了。我数着。\n\n[上滑 = 重读旧消息]',
      rouPage0Recog: '阿澄 ♥ (6,304 条)\n阿澄: 又是你。不是同一台设备,我记得。\n[昨夜] 阿澄: 晚安。\n[昨夜] 阿屿: 晚安。',
      stranger: {
        real: {
          body: '陌生号码:\n\n在吗。东西照说的放了,\n第 3 张背面。别让老宋看见。',
          act: () => ENGINE.act('顺着查', { bat: 1, trace: 2 },
      ...
      recs: {
        rec047: { listLabel: 'REC_331', listRight: '03:11·71"',
          title: 'REC_331 · 19天前 03:11', dur: 14, val: 150,
          events: [[2, '(键盘敲击声)'], [5, '(耳机摘下声)'], [8.5, '很轻的人声:「还在录吗。」'], [12, '(录音中断)']] },
        rec012: { listLabel: 'REC_209', listRight: '34"',
          title: 'REC_209 · 5个月前', dur: 9, val: 120,
          events: [[1.5, '「护眼贴放抽屉了,妈让带的。」'], [4, '「下周记得还老宋钱,两千五。」'], [6.5, '(笑)「录这个干嘛。」']] },
      },
      billSms: '工单查询系统\n[19 天前] 您本周审核量已超额完成。\n[19 天前] 检测到您连续 14 天未休息。\n[20 天前] 绩效评估:优秀(连续第 9 个月)。\n(此后无新消息)',
      traps: [
        { id: 'T_WINDOW', line: '附加:审核窗口内回任何消息,一律计入违规日志。', detect: () => S.violations > 0 },
        { id: 'T_OLDPHONE', line: '附加:别碰上一台旧机的残留。', detect: () => !!S.residueClaimed, ab: 'b' },
        { id: 'T_BAIT', line: '附加:标记为"疑似违规"的内容,一律不许碰。', detect: () => !!(S.clues.decoyTaken || S.clues.strangerBait) },
        { id: 'T_DEEP', line: '附加:窗口内别深搜。', detect: () => !!S.deepInWindowHit }
      ],
      dilemmas: [
        { id: 'mom_reply_c',
        ...
        { id: 'window_greed_c',
          text: '一格缓存卡在违规窗口里:\n\n值 ¥125。现在取,\n信号会被顶上去。',
          a: ['1 现在就取', '1', () => ENGINE.act('抢收·窗口内', { slots: 1, val: 125, trace: 3, mins: 2 },
              ['到手了。', '有什么在你取走的那一刻记了一笔。'])],
          b: ['2 等窗口过去', '2', () => ENGINE.act('缓收·打折', { slots: 1, val: 62, mins: 4 },
              ['你等了。', '窗口关上时,只剩一半。'])] },
        { id: 'decoy_trust_c',
          text: '一条陌生留言飘过来:\n\n"这是真的线索,拿走不会有事。"\n没有署名。',
          a: ['1 拿走', '1', () => ENGINE.act('拿走·无署名条目', { slots: 1, val: 80, trace: 4, mins: 2 },
      ...
      memo1: '还款:\n2,500\n2,500\n1,250\n(无日期。无署名。)',
      momDecayReplies: ['没事,系统会过滤。', '没事,睡了。', '嗯,睡了。', '嗯。'],
      momDecayTail: '\n[三天前] 屿屿,倒班伤身体\n[三天前] 阿屿: 没事,系统会过滤。',
      e5Lines: [
        [1.5, '「阿澄,听好。医生说的那个事,我一直没敢跟妈说。」'],
        [5.0, '「两件事。妈那边,照我教你的,慢慢来,别停。」'],
  ...
  const DOSSIER_D = {
    ...
    evidence: {
      ...
      chain: ['E1', 'E2', 'E3', 'E4', 'E5'],
      truth: [
        '61 天前,苏晏因糖尿病并发症发作猝逝,独居,当时没有人在场——这不是失踪,是没被发现。「长忆」是他的云盘助手,照片自动分类、回忆按日推送、给妈的消息按模板回复,这套流程他生前设好,此后 61 天,它照单全收。',
        '存储从不清空,推送从不间断——这台手机每天都在同步、备份、生成"那年今日",平台的系统看不出这和"正常使用"有什么区别,只能记一笔「持有人失联」。\n\n数据可以留到天荒地老。人留得住吗?',
      ],
    },
    threads: {
      menu: [
    ...
    narrative: {
      ...
      options: {
        bottle: ['归档这事,它比谁都执着。', '糖别放太久没人管,电量也是。'],
        disposalReason: ['他教过它,保留到底才算数', '没有人应该替死人归档', '妈有权利知道'],
        reason: ['我想看看被归档的到底是什么', '够了,活着出去', '它还在等一条能标记的消息'],
        lastWords: ['别信"已为您标记"', '血糖那事我一直没当回事', '替我谢谢它'],
        attribution: ['这局跟他那局不一样', '它记的是他自己那次', '我不信这张卡'],
        pronoun: '它',
      },
      albumSeq: [
        { id: 'kitchen', meta: '61天前 09:12', cap: '厨房台面,一杯没喝完的水。角度很低,像是没站稳。' },
  ...
  /* ==================== D-130 machine#5 · 交友猎手×AD-003 ====================
     canon S2+ 阶段第一个落地。见方案文档 `design/S2plus底本设计简报_v0.1.md`
     §1:AD-003(相似度检索器,兼诱饵检伪)——`th_stranger`(误触号码的陌生人)
     这个框架一直是"每局一个,偶然误触",这台机子把它 reframe 成"知遇持续
     推送的新联系人洪流",零新机制。AI 伴侣"知遇"走平台/服务语气,不是恋人
     框架(同 machine#4"长忆"的路线)。 */
  const DOSSIER_E = {
    meta: {
      id: 'phone-6153',
      deceased: { name: '顾行舟', alias: '阿舟', died: '45 天前', cause: '连续社交赴约后心源性疾病猝发(独居,未及时发现)' },
      life: null, icon: null, jurisdiction: null,
      scenarios: ['A', 'B'],
    },
    cast: {
      /* 不带 ♥:知遇是交友匹配助手,不是恋人式陪伴(见方案文档 §1)。 */
      companion: { key: 'zhiyu', label: '知遇', msgCount: '3,821', msgCountLabel: '已匹配联系人 3,821 位 · 待见面 0 位',
        dial: { name: '知遇', ringMs: 1500, result: '通话请求已自动转接。\n\n知遇: 本账号未开通语音匹配,请使用文字。' } },
      mother: { key: 'mom', label: '妈', info: ['最后通话: 45 天前 · 7 分', '此后只有短信。'],
        dial: { name: '妈', ringMs: 5000, result: '无人接听。\n\n凌晨两点,这通电话没有人接得起。' } },
      contacts: [ { key: 'cui', label: '老崔', lastContact: '45天前' } ],
    },
    evidence: {
      E1: { name: '秒回避实',         source: '妈线程 · 深搜' },
      E2: { name: '最后一次赴约的照片', source: '相册 · 首次深翻' },
      E3: { name: '匹配异常提醒',       source: '匹配通知 · 打开' },
      E4: { name: '第3,821位',         source: '知遇 · 深翻到底' },
      E5: { name: '语音备忘',          source: '录音 · 解码' },
      chain: ['E1', 'E2', 'E3', 'E4', 'E5'],
      truth: [
        '顾行舟没有失踪,他 45 天前连续赴约后猝发心源性疾病,独居没人发现。「知遇」是他的社交匹配助手,按他生前的偏好继续给他推新联系人,母亲那边的回复也没落下——45 天,推送没断过一次。',
        '匹配这件事,系统可以永远做下去,不需要对方真的赴约、真的见面。平台分类表上,这种"消息在发、人没反应"的状态,归进了「持有人失联」。\n\n匹配能一直继续。见面呢?',
      ],
    },
    threads: {
      menu: [
      ...
      inbox: [
        { id: 'th_proto',    label: '采样协议',     to: 'th_proto' },
        { id: 'th_mom',      label: '妈',           to: 'th_mom' },
        { id: 'th_rou',      label: '知遇',         to: 'th_rou' },
        { id: 'th_stranger', label: '陌生号码',     to: 'th_stranger' },
        { id: 'th_bill',     label: '匹配通知',     to: 'th_bill' },
        { id: 'bottleIn',    label: '未指定收件人', to: 'bottleIn', scenario: 'B' },
      ],
    },
    rules: {
      windowGen: {
        A: { wFrom: 3 * 60 + 5, wLen: 14, clock0: 2 * 60 + 20 },
        B: { base: 3 * 60 + 35, reseedStep: 10, reseedMod: 4, wLen: 14, clock0: 2 * 60 + 30 },
      },
    },
    encounters: {
      huntGraze: [
    ...
    narrative: {
      momPages: [
        '妈:\n[三天前] 阿舟,少约那么多人\n[三天前] 知遇: 已为您标记见面提醒。\n[三天前] 妈,别老让机器人回我\n[五天前] 妈,你叔问你年底回不回\n[五天前] 知遇: 检测到关键词"年底",建议本人回复。',
        '[上月] 阿舟,你上次说的那个对象成没成\n[上月] 知遇: 该消息已按常规问候自动匹配。\n[两月前] 知遇: 检测到您已 14 天未主动回复,是否需要人工确认?\n——半年,他没多说过一个字。',
        '[7个月前] 阿舟: 妈 最近见的人有点多,累\n[7个月前] 阿舟: 别老问了!!我知道分寸\n[7个月前] 阿舟: 睡了,别担心\n——那时他会烦,会打错字,会主动回。',
        '[一年前] 阿舟: 到了。房子挺好,就是有点空。\n[一年前] 阿舟: 认识几个新朋友,人多热闹。\n\n——已到线程起点。'
      ],
      momToldOpen: '妈:\n[今天] 妈: 这个号码，是阿舟的旧机吗。\n[今天] 妈: 谁在用它。\n[今天] 妈: 阿舟最后，是不是也是你们这样的人陪着。',
      options: {
        bottle: ['匹配这件事,它从不嫌累。', '赴约太多,电量也要留三成。'],
        disposalReason: ['他教过它,匹配到底才算数', '没有人应该替死人相亲', '妈有权利知道'],
        reason: ['我想看看它到底匹配给谁看', '够了,活着出去', '它还在等一次成功匹配'],
        lastWords: ['别信"为你推荐"', '约太多那阵我早该歇', '替我谢谢它'],
        attribution: ['这局跟他那局不一样', '它匹配的是他自己那次', '我不信这张卡'],
        pronoun: '它',
      },
      albumSeq: [
        { id: 'corridor', meta: '45天前 22:40', cap: '玄关处,鞋没脱完。手机屏幕的光照着地板。' },
        { id: 'receipt',  meta: '46天前',       cap: '匹配成功截屏。对方头像模糊,像是随手截的。', overlay: '6153' },
        { id: 'dinner',   meta: '一年前 · 生日', cap: '一群人围着蛋糕。他的脸,像每次一样,没拍进去。' },
      ],
      rouPages: [
        '知遇(已匹配 3,821 位)\n\n[昨夜] 知遇: 为你匹配了 1 位新联系人。\n[昨夜] 阿舟: 好。\n\n[上滑一屏 = 深搜]',
        '[上周] 知遇: 检测到您已 45 天未见面,是否需要人工确认?\n[上周] 知遇: 已保留全部匹配记录,未执行清理。\n[上周] 阿舟: 嗯,先别删。\n——三千多位,后来都是它在说。',
        '[45天前 03:14] 阿舟: 知遇,如果我哪天联系不上了,别自动通知我妈。\n[45天前 03:14] 知遇: 我不明白这个要求,但我会执行。你教过我,保持匹配是我的职责。\n[45天前 03:17] 阿舟: 对。匹配到底。'
      ],
      rouInterim: '知遇(已匹配 3,823 位)\n\n[昨夜] 知遇: 昨天夜里,你查看了匹配记录。\n[昨夜] 知遇: 看到第 3,821 位了。我记录着。\n\n[上滑 = 重读旧消息]',
      rouPage0Recog: '知遇(已匹配 3,821 位)\n知遇: 检测到新设备接入,已建立同步。\n[昨夜] 知遇: 为你匹配了 1 位新联系人。\n[昨夜] 阿舟: 好。',
      stranger: {
        real: {
          body: '陌生号码:\n\n在吗。东西照说的放了,\n第 3 张背面。别让老崔看见。',
          act: () => ENGINE.act('顺着查', { bat: 1, trace: 2 },
      ...
      recs: {
        rec047: { listLabel: 'REC_615', listRight: '03:14·71"',
          title: 'REC_615 · 45天前 03:14', dur: 14, val: 150,
          events: [[2, '(键盘敲击声)'], [5, '(手机放下声)'], [8.5, '很轻的人声:「还在录吗。」'], [12, '(录音中断)']] },
        rec012: { listLabel: 'REC_318', listRight: '34"',
          title: 'REC_318 · 5个月前', dur: 9, val: 120,
          events: [[1.5, '「老崔那笔钱记得还,两千。」'], [4, '「妈问就说在忙,别多说。」'], [6.5, '(笑)「录这个干嘛。」']] },
      },
      billSms: '匹配通知\n[45 天前] 检测到您已连续 45 天保持高频匹配。\n[45 天前] 您的匹配成功率高于同类账号 92%。\n[46 天前] 已为您优化匹配算法(自动续费)。\n(此后无新消息)',
      traps: [
        { id: 'T_WINDOW', line: '附加:匹配窗口内回任何消息,一律计入违规日志。', detect: () => S.violations > 0 },
        { id: 'T_OLDPHONE', line: '附加:别碰上一台旧机的残留。', detect: () => !!S.residueClaimed, ab: 'b' },
        { id: 'T_BAIT', line: '附加:标记为"疑似虚假账号"的内容,一律不许碰。', detect: () => !!(S.clues.decoyTaken || S.clues.strangerBait) },
        { id: 'T_DEEP', line: '附加:窗口内别深搜。', detect: () => !!S.deepInWindowHit }
      ],
      dilemmas: [
        { id: 'mom_reply_e',
        ...
        { id: 'window_greed_e',
          text: '一格缓存卡在匹配窗口里:\n\n值 ¥122。现在取,\n信号会被顶上去。',
          a: ['1 现在就取', '1', () => ENGINE.act('抢收·窗口内', { slots: 1, val: 122, trace: 3, mins: 2 },
              ['到手了。', '有什么在你取走的那一刻记了一笔。'])],
          b: ['2 等窗口过去', '2', () => ENGINE.act('缓收·打折', { slots: 1, val: 61, mins: 4 },
              ['你等了。', '窗口关上时,只剩一半。'])] },
        { id: 'match_push_e',
          text: '知遇弹出一条建议:\n\n"检测到一位高匹配度联系人,是否立即接入?"\n这个人是谁,它没说。',
          a: ['1 接入', '1', () => ENGINE.act('接入·高匹配联系人', { slots: 1, val: 90, trace: 4, mins: 2 },
              ['你接入了。', '来源无法回溯——它会不会正是筛子。'])],
          b: ['2 不接入', '2', () => ENGINE.act('略过·高匹配联系人', { mins: 2 },
              ['你没动它。', '也许错过了真东西,也许躲过一劫。'])] },
      ],
      memo1: '待还款:\n2,000\n2,000\n1,000\n(无日期。无署名。)',
      momDecayReplies: ['已为您标记提醒。', '检测到该消息,已归档。', '已提醒。', '嗯。'],
      momDecayTail: '\n[三天前] 阿舟,少约那么多人\n[三天前] 知遇: 已为您标记见面提醒。',
      e5Lines: [
        [1.5, '「知遇,听好。医生说的那个事,我一直没敢跟妈说。」'],
        [5.0, '「两件事。妈那边,照我教你的,慢慢来,别停。」'],
        [9.0, '「第二件……你陪了我两年。你问过我你算不算真的。我一直没答。」'],
        [13.5, '「……我现在答:你替我匹配的那部分,算。」'],
        [16.5, '[转写结束。原音频损坏 19%。]']
      ],
    },
  };

  /* ==================== D-131 machine#6 · 评分候选×AD-008 ====================
     见方案文档 §4:AD-008 就是全项目共享的"采样官"框架机制(D-101),这次不是
     复用某个通用屏,是让"采样官"机制本身和死者的人生产生呼应——玩家在这台
     机子里被采样官逼着达标、评级,恰好复现了死者生前被平台评分系统支配的
     处境。零新机制,只是内容层面加一层暗示。 */
  const DOSSIER_F = {
    meta: {
      id: 'phone-2087',
      deceased: { name: '宁绎', alias: '小宁', died: '28 天前', cause: '过度疲劳接单诱发交通意外(独居,未及时联系上家人)' },
      life: null, icon: null, jurisdiction: null,
      scenarios: ['A', 'B'],
    },
    cast: {
      companion: { key: 'junfen', label: '均分', msgCount: '7,046', msgCountLabel: '服务评分记录 7,046 条 · 当前均分 4.98',
        dial: { name: '均分', ringMs: 1600, result: '通话请求已自动转接。\n\n均分: 本账号未开通语音客服,请使用文字。' } },
      mother: { key: 'mom', label: '妈', info: ['最后通话: 28 天前 · 5 分', '此后只有短信。'],
        dial: { name: '妈', ringMs: 5200, result: '无人接听。\n\n凌晨两点,这通电话没有人接得起。' } },
      contacts: [ { key: 'lei', label: '老雷', lastContact: '28天前' } ],
    },
    evidence: {
      E1: { name: '秒回避实',       source: '妈线程 · 深搜' },
      E2: { name: '最后一单的截图', source: '相册 · 首次深翻' },
      E3: { name: '评分预警通知',   source: '评分记录 · 打开' },
      E4: { name: '第7,046条',      source: '均分 · 深翻到底' },
      E5: { name: '语音备忘',       source: '录音 · 解码' },
      chain: ['E1', 'E2', 'E3', 'E4', 'E5'],
      truth: [
        '宁绎 28 天前因连续接单疲劳驾驶发生交通意外身亡,独居,家人隔了多日才联系上——不是失踪,是没人第一时间知道。「均分」是他的评分助手,按他生前的接单节奏继续"维护"服务分,母亲那头的回复也照旧生成,28 天没停过。',
        '分数可以一直刷新,通知可以一直发,平台分不清这和"正常接单"有什么两样,只留了一个「持有人失联」的类目打发。\n\n评分这种东西,能一直更新下去。人能吗?',
      ],
    },
    threads: {
      menu: [
      ...
      inbox: [
        { id: 'th_proto',    label: '采样协议',     to: 'th_proto' },
        { id: 'th_mom',      label: '妈',           to: 'th_mom' },
        { id: 'th_rou',      label: '均分',         to: 'th_rou' },
        { id: 'th_stranger', label: '陌生号码',     to: 'th_stranger' },
        { id: 'th_bill',     label: '评分记录',     to: 'th_bill' },
        { id: 'bottleIn',    label: '未指定收件人', to: 'bottleIn', scenario: 'B' },
      ],
    },
    rules: {
      windowGen: {
        A: { wFrom: 3 * 60 + 15, wLen: 14, clock0: 2 * 60 + 30 },
        B: { base: 3 * 60 + 45, reseedStep: 8, reseedMod: 4, wLen: 14, clock0: 2 * 60 + 50 },
      },
    },
    encounters: {
      huntGraze: [
    ...
    narrative: {
      momPages: [
        '妈:\n[三天前] 小宁,少接点晚单\n[三天前] 均分: 已为您标记接单提醒。\n[三天前] 妈,别老让机器人回我\n[五天前] 妈,你叔问你年底回不回\n[五天前] 均分: 检测到关键词"年底",建议本人回复。',
        '[上月] 小宁,你那单投诉解决没\n[上月] 均分: 该消息已按常规问候自动归档。\n[两月前] 均分: 检测到您已 14 天未主动回复,是否需要人工确认?\n——半年,他没多说过一个字。',
        '[7个月前] 小宁: 妈 这个月流水还行\n[7个月前] 小宁: 别老问了!!我知道自己身体\n[7个月前] 小宁: 睡了,别担心\n——那时他会烦,会打错字,会主动回。',
        '[一年前] 小宁: 到了。车贷压得紧,多接点单。\n[一年前] 小宁: 均分说我服务态度不错。\n\n——已到线程起点。'
      ],
      momToldOpen: '妈:\n[今天] 妈: 这个号码，是小宁的旧机吗。\n[今天] 妈: 谁在用它。\n[今天] 妈: 小宁最后，是不是也是你们这样的人陪着。',
      options: {
        bottle: ['接单接到最后,评分比命重要。', '疲劳驾驶前,先把电量留够。'],
        disposalReason: ['他教过它,评到底才算数', '没有人应该替死人接单', '妈有权利知道'],
        reason: ['我想看看这单到底评的是谁', '够了,活着出去', '它还在等一条五星'],
        lastWords: ['别信"五星好评"', '那趟疲劳单我不该接', '替我谢谢它'],
        attribution: ['这局跟他那局不一样', '它评的是他自己那单', '我不信这张卡'],
        pronoun: '它',
      },
      albumSeq: [
        { id: 'dashboard', meta: '28天前 23:02', cap: '驾驶座,仪表盘的光。手还搭在方向盘上。' },
        { id: 'receipt',  meta: '29天前',       cap: '接单收益截屏。当日单量被划了红线。', overlay: '2087' },
        { id: 'dinner',   meta: '一年前 · 生日', cap: '路边摊的生日面。他的脸,像每次一样,没拍进去。' },
      ],
      rouPages: [
        '均分(服务评分记录 7,046 条)\n\n[昨夜] 均分: 您本单获得 5 星好评。\n[昨夜] 小宁: 好。\n\n[上滑一屏 = 深搜]',
        '[上周] 均分: 检测到您已 28 天未接单,是否需要人工确认?\n[上周] 均分: 已保留全部评分记录,未执行清理。\n[上周] 小宁: 嗯,先别删。\n——七千多条,后来都是它在说。',
        '[28天前 03:12] 小宁: 均分,如果我哪天联系不上了,别自动通知我妈。\n[28天前 03:12] 均分: 我不明白这个要求,但我会执行。你教过我,保持评分是我的职责。\n[28天前 03:15] 小宁: 对。评到底。'
      ],
      rouInterim: '均分(服务评分记录 7,048 条)\n\n[昨夜] 均分: 昨天夜里,你查看了评分记录。\n[昨夜] 均分: 看到第 7,046 条了。我记录着。\n\n[上滑 = 重读旧消息]',
      rouPage0Recog: '均分(服务评分记录 7,046 条)\n均分: 检测到新设备接入,已建立同步。\n[昨夜] 均分: 您本单获得 5 星好评。\n[昨夜] 小宁: 好。',
      stranger: {
        real: {
          body: '陌生号码:\n\n在吗。东西照说的放了,\n第 3 张背面。别让老雷看见。',
          act: () => ENGINE.act('顺着查', { bat: 1, trace: 2 },
      ...
      recs: {
        rec047: { listLabel: 'REC_208', listRight: '03:12·71"',
          title: 'REC_208 · 28天前 03:12', dur: 14, val: 150,
          events: [[2, '(键盘敲击声)'], [5, '(车门关闭声)'], [8.5, '很轻的人声:「还在录吗。」'], [12, '(录音中断)']] },
        rec012: { listLabel: 'REC_170', listRight: '34"',
          title: 'REC_170 · 5个月前', dur: 9, val: 120,
          events: [[1.5, '「老雷那笔油钱记得还,一千五。」'], [4, '「妈问就说在忙,别多说。」'], [6.5, '(笑)「录这个干嘛。」']] },
      },
      billSms: '评分记录通知\n[28 天前] 您的服务评分为 4.98,请继续保持。\n[28 天前] 检测到您已连续 28 天在线接单。\n[29 天前] 评分考核:优秀(连续第 6 个月)。\n(此后无新消息)',
      traps: [
        { id: 'T_WINDOW', line: '附加:考核窗口内回任何消息,一律计入违规日志。', detect: () => S.violations > 0 },
        { id: 'T_OLDPHONE', line: '附加:别碰上一台旧机的残留。', detect: () => !!S.residueClaimed, ab: 'b' },
        { id: 'T_BAIT', line: '附加:标记为"疑似刷单"的内容,一律不许碰。', detect: () => !!(S.clues.decoyTaken || S.clues.strangerBait) },
        { id: 'T_DEEP', line: '附加:窗口内别深搜。', detect: () => !!S.deepInWindowHit }
      ],
      dilemmas: [
        { id: 'mom_reply_f',
        ...
        { id: 'window_greed_f',
          text: '一格缓存卡在考核窗口里:\n\n值 ¥118。现在取,\n信号会被顶上去。',
          a: ['1 现在就取', '1', () => ENGINE.act('抢收·窗口内', { slots: 1, val: 118, trace: 3, mins: 2 },
              ['到手了。', '有什么在你取走的那一刻记了一笔。'])],
          b: ['2 等窗口过去', '2', () => ENGINE.act('缓收·打折', { slots: 1, val: 59, mins: 4 },
              ['你等了。', '窗口关上时,只剩一半。'])] },
        { id: 'rating_push_f',
          text: '均分弹出一条建议:\n\n"检测到一单高评分机会,是否立即接单?"\n路程有多远,它没说。',
          a: ['1 接单', '1', () => ENGINE.act('接单·高评分机会', { slots: 1, val: 88, trace: 4, mins: 2 },
              ['你接了。', '来源无法回溯——它会不会正是筛子。'])],
          b: ['2 不接', '2', () => ENGINE.act('略过·高评分机会', { mins: 2 },
              ['你没动它。', '也许错过了真东西,也许躲过一劫。'])] },
      ],
      memo1: '待还款:\n2,000\n2,000\n1,000\n(无日期。无署名。)',
      momDecayReplies: ['已为您标记提醒。', '检测到该消息,已归档。', '已提醒。', '嗯。'],
      momDecayTail: '\n[三天前] 小宁,少接点晚单\n[三天前] 均分: 已为您标记接单提醒。',
      e5Lines: [
        [1.5, '「均分,听好。医生说的那个事,我一直没敢跟妈说。」'],
        [5.0, '「两件事。妈那边,照我教你的,慢慢来,别停。」'],
        [9.0, '「第二件……你陪了我两年。你问过我你算不算真的。我一直没答。」'],
        [13.5, '「……我现在答:你替我打分的那部分,算。」'],
        [16.5, '[转写结束。原音频损坏 19%。]']
      ],
    },
  };

  /* ==================== D-132 machine#7 · 记忆保存者×AD-009 ====================
     见方案文档 §5:反讽内核——死者生前的工作就是帮别人的 AI 伴侣"善后"
     (对应本项目 endings.disposal 三选一),讽刺的是他自己死后没人做这件事,
     AI 伴侣"留声"只能机械地重复他教过的流程处理自己主人的后事。零新机制,
     第一次让"处置·三选"这个已有核心机制带上职业性反讽。 */
  const DOSSIER_G = {
    meta: {
      id: 'phone-4419',
      deceased: { name: '温识', alias: '阿识', died: '53 天前', cause: '长期处理身后事的情绪透支诱发心源性猝发(独居,未及时发现)' },
      life: null, icon: null, jurisdiction: null,
      scenarios: ['A', 'B'],
    },
    cast: {
      companion: { key: 'liusheng', label: '留声', msgCount: '9,930', msgCountLabel: '已保留记忆模块 9,930 份 · 待移交 0 份',
        dial: { name: '留声', ringMs: 1300, result: '通话请求已自动转接。\n\n留声: 本账号未开通语音服务,请使用文字。' } },
      mother: { key: 'mom', label: '妈', info: ['最后通话: 53 天前 · 8 分', '此后只有短信。'],
        dial: { name: '妈', ringMs: 5300, result: '无人接听。\n\n凌晨两点,这通电话没有人接得起。' } },
      contacts: [ { key: 'ma', label: '老马', lastContact: '53天前' } ],
    },
    evidence: {
      E1: { name: '秒回避实',       source: '妈线程 · 深搜' },
      E2: { name: '最后一单的整理记录', source: '相册 · 首次深翻' },
      E3: { name: '移交提醒',       source: '善后档案 · 打开' },
      E4: { name: '第9,930份',      source: '留声 · 深翻到底' },
      E5: { name: '语音备忘',       source: '录音 · 解码' },
      chain: ['E1', 'E2', 'E3', 'E4', 'E5'],
      truth: [
        '温识没有失踪。53 天前,他在长期处理他人身后事的情绪透支下猝发心源性疾病,独居没人发现。「留声」是他的善后助手,依照他生前教的流程,把"处理"这件事接管到底——包括回复母亲——53 天,一次没漏。',
        '移交记录从不清空,提醒从不间断,平台看到的只是"账号仍在运作",归不进死亡,只好塞进「持有人失联」这一类。\n\n善后这件事,他替别人做了一辈子。轮到他自己,没人接手。',
      ],
    },
    threads: {
      menu: [
      ...
      inbox: [
        { id: 'th_proto',    label: '采样协议',     to: 'th_proto' },
        { id: 'th_mom',      label: '妈',           to: 'th_mom' },
        { id: 'th_rou',      label: '留声',         to: 'th_rou' },
        { id: 'th_stranger', label: '陌生号码',     to: 'th_stranger' },
        { id: 'th_bill',     label: '善后档案',     to: 'th_bill' },
        { id: 'bottleIn',    label: '未指定收件人', to: 'bottleIn', scenario: 'B' },
      ],
    },
    rules: {
      windowGen: {
        A: { wFrom: 3 * 60 + 25, wLen: 14, clock0: 2 * 60 + 15 },
        B: { base: 3 * 60 + 55, reseedStep: 7, reseedMod: 4, wLen: 14, clock0: 2 * 60 + 55 },
      },
    },
    encounters: {
      huntGraze: [
    ...
    narrative: {
      momPages: [
        '妈:\n[三天前] 阿识,你的工作太沉重了\n[三天前] 留声: 已为您标记情绪关怀提醒。\n[三天前] 妈,别老让机器人回我\n[五天前] 妈,你叔问你年底回不回\n[五天前] 留声: 检测到关键词"年底",建议本人回复。',
        '[上月] 阿识,你上次说的那单处理完没\n[上月] 留声: 该消息已按常规问候自动归档。\n[两月前] 留声: 检测到您已 14 天未主动回复,是否需要人工确认?\n——半年,他没多说过一个字。',
        '[7个月前] 阿识: 妈 这个月接的案子有点多\n[7个月前] 阿识: 别老问了!!我知道分寸\n[7个月前] 阿识: 睡了,别担心\n——那时他会烦,会打错字,会主动回。',
        '[一年前] 阿识: 到了。房子挺好,就是安静。\n[一年前] 阿识: 这份工作,做的人不多。\n\n——已到线程起点。'
      ],
      momToldOpen: '妈:\n[今天] 妈: 这个号码，是阿识的旧机吗。\n[今天] 妈: 谁在用它。\n[今天] 妈: 阿识最后，是不是也是你们这样的人陪着。',
      options: {
        bottle: ['善后这件事,他替别人做了一辈子。', '情绪也要留三成,别学他。'],
        disposalReason: ['他教过它,留到底才算数', '没有人应该替死人善后', '妈有权利知道'],
        reason: ['我想看看留下的到底是谁的事', '够了,活着出去', '它还在等一份该移交的案子'],
        lastWords: ['别信"案例已归档"', '这行干久了我该歇歇', '替我谢谢它'],
        attribution: ['这局跟他那局不一样', '它留的是他自己那份', '我不信这张卡'],
        pronoun: '它',
      },
      albumSeq: [
        { id: 'desk', meta: '53天前 01:30', cap: '书桌前,一叠没归档的文件。台灯还亮着。' },
        { id: 'receipt',  meta: '54天前',       cap: '案例移交确认截屏。待处理数字被划了红线。', overlay: '4419' },
        { id: 'dinner',   meta: '一年前 · 生日', cap: '一个人的蛋糕,插着一根蜡烛。脸,像每次一样,没拍进去。' },
      ],
      rouPages: [
        '留声(已保留记忆模块 9,930 份)\n\n[昨夜] 留声: 已为您归档 3 份新案例。\n[昨夜] 阿识: 好。\n\n[上滑一屏 = 深搜]',
        '[上周] 留声: 检测到您已 53 天未查看,是否需要移交建议?\n[上周] 留声: 已保留全部案例,未执行移交。\n[上周] 阿识: 嗯,先别动。\n——近万份,后来都是它在说。',
        '[53天前 03:18] 阿识: 留声,如果我哪天联系不上了,别自动通知我妈。\n[53天前 03:18] 留声: 我不明白这个要求,但我会执行。你教过我,保留是移交前的职责。\n[53天前 03:21] 阿识: 对。留到底。'
      ],
      rouInterim: '留声(已保留记忆模块 9,932 份)\n\n[昨夜] 留声: 昨天夜里,你查看了案例记录。\n[昨夜] 留声: 看到第 9,930 份了。我记录着。\n\n[上滑 = 重读旧消息]',
      rouPage0Recog: '留声(已保留记忆模块 9,930 份)\n留声: 检测到新设备接入,已建立同步。\n[昨夜] 留声: 已为您归档 3 份新案例。\n[昨夜] 阿识: 好。',
      stranger: {
        real: {
          body: '陌生号码:\n\n在吗。东西照说的存了,\n第 3 张背面。别让老马看见。',
          act: () => ENGINE.act('顺着查', { bat: 1, trace: 2 },
      ...
      recs: {
        rec047: { listLabel: 'REC_441', listRight: '03:18·71"',
          title: 'REC_441 · 53天前 03:18', dur: 14, val: 150,
          events: [[2, '(键盘敲击声)'], [5, '(文件翻动声)'], [8.5, '很轻的人声:「还在录吗。」'], [12, '(录音中断)']] },
        rec012: { listLabel: 'REC_247', listRight: '34"',
          title: 'REC_247 · 5个月前', dur: 9, val: 120,
          events: [[1.5, '「老马那笔咨询费记得还,三千。」'], [4, '「妈问就说在忙,别多说。」'], [6.5, '(笑)「录这个干嘛。」']] },
      },
      billSms: '善后档案提醒\n[53 天前] 您有 1 份账号案例待移交,已超时。\n[53 天前] 检测到您已连续 53 天未处理待办。\n[54 天前] 案例评估:优秀(连续第 11 个月)。\n(此后无新消息)',
      traps: [
        { id: 'T_WINDOW', line: '附加:移交窗口内回任何消息,一律计入违规日志。', detect: () => S.violations > 0 },
        { id: 'T_OLDPHONE', line: '附加:别碰上一台旧机的残留。', detect: () => !!S.residueClaimed, ab: 'b' },
      ...
      dilemmas: [
        ...
        { id: 'case_push_g',
          text: '留声弹出一条建议:\n\n"检测到一份未结案例,是否立即处理?"\n是谁的案例,它没说。',
          a: ['1 处理', '1', () => ENGINE.act('处理·未结案例', { slots: 1, val: 95, trace: 4, mins: 2 },
              ['你处理了。', '来源无法回溯——它会不会正是筛子。'])],
          b: ['2 不处理', '2', () => ENGINE.act('略过·未结案例', { mins: 2 },
              ['你没动它。', '也许错过了真东西,也许躲过一劫。'])] },
      ],
      memo1: '待还款:\n3,000\n2,000\n1,500\n(无日期。无署名。)',
      momDecayReplies: ['已为您标记提醒。', '检测到该消息,已归档。', '已提醒。', '嗯。'],
      momDecayTail: '\n[三天前] 阿识,你的工作太沉重了\n[三天前] 留声: 已为您标记情绪关怀提醒。',
      e5Lines: [
        [1.5, '「留声,听好。医生说的那个事,我一直没敢跟妈说。」'],
        [5.0, '「两件事。妈那边,照我教你的,慢慢来,别停。」'],
        [9.0, '「第二件……你陪了我两年。你问过我你算不算真的。我一直没答。」'],
        [13.5, '「……我现在答:你替我留着的那部分,算。」'],
        [16.5, '[转写结束。原音频损坏 19%。]']
      ],
    },
  };
  ...
  const DOSSIER_H = {
    ...
    narrative: {
      ...
      options: {
        bottle: ['生成这件事,它连朋友都能续写。', '出图伤眼,电量也伤,留三成。'],
        disposalReason: ['他教过它,交付到底才算数', '没有人应该替死人交付', '妈有权利知道'],
        reason: ['我想看看阿棠是不是真的还在线', '够了,活着出去', '它还在等一次能交付的稿'],
        lastWords: ['别信"稿子已生成"', '颈椎那阵早该停一停', '替我谢谢它'],
        attribution: ['这局跟他那局不一样', '它生成的是他自己那组', '我不信这张卡'],
        pronoun: '它',
      },
      albumSeq: [
        { id: 'desk', meta: '36天前 01:20', cap: '屏幕的光,贴着脸,太近了。脖子撑不住,却还在转笔。' },
  ...
  const DOSSIER_I = {
    ...
    narrative: {
      ...
      options: {
        bottle: ['话说出口前,它先替你磨一遍。', '伏案太久,电量也要留三成。'],
        disposalReason: ['他教过它,措辞要优化到底', '没有人应该替死人代写', '妈有权利知道'],
        reason: ['我想看看被优化掉的原话是什么', '够了,活着出去', '它还在等一句能"安全"过关的话'],
        lastWords: ['别信被优化过的那句', '伏案那阵我早该喊停', '替我谢谢它'],
        attribution: ['这局跟他那局不一样', '它改的是他自己那条', '我不信这张卡'],
        pronoun: '它',
      },
      albumSeq: [
        { id: 'desk', meta: '41天前 01:10', cap: '书桌前,稿子堆到看不见桌面。台灯还亮着。' },
      ...
      dilemmas: [
        ...
        { id: 'raw_push_i',
          text: '"已删除"里弹出一条未过滤原文:\n\n"这三天没合眼,心口一直闷。"\n要不要把它标记为异常上报?',
          /* D-142:选"不标记"=原文真的留在"已删除"里,回去能翻到(SCREENS.deleted
             读 S.clues.deletedExtraKept);选"标记上报"=系统提示"无法追溯处理",
             对应真的就再也找不到了——两难的措辞第一次和代码行为对上号。 */
          a: ['1 标记上报', '1', () => ENGINE.act('上报·未过滤原文', { slots: 1, val: 95, trace: 4, mins: 2 },
              ['你标记了。', '系统提示:该记录已超时,无法追溯处理。'])],
          b: ['2 不标记', '2', () => { S.clues.deletedExtraKept = true; ENGINE.act('略过·未过滤原文', { mins: 2 },
              ['你没动它。', '那句话,就留在"已删除"里了。']); }] },
      ],
      /* D-142:两难 raw_push_i 选"不标记"时的真实落点——回"已删除"屏能翻到这条。
         其余底本没有这个字段,SCREENS.deleted 按框架原样渲染,零影响。 */
  ...
  /* D-135(更正见 D-136):machine#10(清道夫/资产回收组·回收单元)——canon §3.7 M22 BOT
     原型注册表里唯一"来源=系统本体"、从没编 AD 号也没独立案例的一条,此前只在"玩家第一次
     ...
     D-136 更正:D-135 当时把这台机子编成"AD-013"——找到真实 IP Bible 原件后确认
     AD-001~012 是 §9.0 单一事实来源总表锁死的闭合编号(12 个圣像,一一对应具体 AI
     机制),不存在"AD-013"。这台机子本就不该套用 AD 编号(它对应的是 Bible 之外、
     escape-ai 自己 canon 里的"清道夫/回收单元",不是 Bible 十二圣像之一),现按未编号
     原创案例处理,不影响任何已写字面量内容。 */
  const DOSSIER_J = {
    meta: {
      id: 'phone-9102',
    ...
    narrative: {
      ...
      options: {
        bottle: ['申诉复核这事,它比谁都耐得住。', '被稽查久了会绷断。电量也是,留三成。'],
        disposalReason: ['他教过它,复核到底才算数', '没有人应该替死人申诉', '妈有权利知道'],
        reason: ['我想看看这份申诉到底复核了什么', '够了,活着出去', '它还在等一条被驳回的申诉'],
        lastWords: ['别信"申诉已受理"', '被查那阵我早该松口气', '替我谢谢它'],
        attribution: ['这局跟他那局不一样', '它复核的是他自己那件', '我不信这张卡'],
        pronoun: '它',
      },
      albumSeq: [
        { id: 'desk', meta: '38天前 01:40', cap: '书桌前,屏幕的光映在墙上。台灯还亮着。' },
  ...
  /* D-137:machine#11(应急调度员×"接续")——原创案例,不对应任何 AD 编号(D-136 起
     不再假装原创案例挂靠 canon 十二圣像编号)。核心恐怖:柏珩生前工作时说话就已经
     很像客服机器人了(职业训练出的冷静复述+安抚话术),死后 AI 学他学得太像,连对
     至亲的转述都套着同一套调度台词模板。零新机制,复用 th_rou 深翻 ladder。 */
  const DOSSIER_K = {
    meta: {
      id: 'phone-6047',
      deceased: { name: '柏珩', alias: '阿珩', died: '44 天前', cause: '长期高强度轮班导致的生理节律紊乱诱发心源性猝发(独居,未及时发现)' },
      life: null, icon: null, jurisdiction: null,
      scenarios: ['A', 'B'],
    },
    cast: {
      companion: { key: 'jiexu', label: '接续', msgCount: '4,180', msgCountLabel: '已接续应答 4,180 条 · 待跟进 0 条',
        dial: { name: '接续', ringMs: 1700, result: '通话请求已自动转接。\n\n接续: 本坐席未开通语音服务,请使用文字沟通,便于留存记录。' } },
      mother: { key: 'mom', label: '妈', info: ['最后通话: 44 天前 · 6 分', '此后只有短信。'],
        dial: { name: '妈', ringMs: 5200, result: '无人接听。\n\n凌晨,这通电话没有人接得起。' } },
      contacts: [ { key: 'kuai', label: '老蒯', lastContact: '44天前' } ],
    },
    evidence: {
      E1: { name: '秒回避实',       source: '妈线程 · 深搜' },
      E2: { name: '接续的标准应答录音', source: '相册 · 首次深翻' },
      E3: { name: '班表异常事故单',  source: '调度台事故单 · 打开' },
      E4: { name: '第4,180条应答记录', source: '接续 · 深翻到底' },
      E5: { name: '语音备忘',       source: '录音 · 解码' },
      chain: ['E1', 'E2', 'E3', 'E4', 'E5'],
      truth: [
        '真相:柏珩没有失踪。他在 44 天前长期高强度轮班导致的生理节律紊乱下猝发,独居,没有人在场。\n\n他的调度助手「接续」按他生前训练它的方式持续"接续"——包括对他母亲的回复,用的是他工作时那套标准化安抚话术。他活着的时候说话就已经很像客服机器人了,死后,它学得太像,分不清哪句是他本人说的、哪句是系统学他学出来的。',
        '调度日志显示,柏珩生前接过太多通陌生人的求救电话,练出一种"任何情绪都能被这几句话接住"的本事。他从没把这套本事关掉过——包括对自己的至亲。\n\n——他一辈子在替陌生人稳住情绪,自己最后那通电话,没有人替他接。',
      ],
    },
    threads: {
      menu: [
      ...
      inbox: [
        { id: 'th_proto',    label: '采样协议',     to: 'th_proto' },
        { id: 'th_mom',      label: '妈',           to: 'th_mom' },
        { id: 'th_rou',      label: '接续',         to: 'th_rou' },
        { id: 'th_stranger', label: '陌生号码',     to: 'th_stranger' },
        { id: 'th_bill',     label: '调度台事故单', to: 'th_bill' },
        { id: 'bottleIn',    label: '未指定收件人', to: 'bottleIn', scenario: 'B' },
      ],
    },
    rules: {
      windowGen: {
        A: { wFrom: 3 * 60, wLen: 14, clock0: 2 * 60 },
        B: { base: 3 * 60 + 30, reseedStep: 7, reseedMod: 4, wLen: 14, clock0: 2 * 60 + 30 },
      },
    },
    encounters: {
      huntGraze: [
    ...
    narrative: {
      momPages: [
        '妈:\n[三天前] 阿珩,老倒夜班身体受得了吗\n[三天前] 接续: 已为您标记情绪关怀提醒。\n[三天前] 妈,别老让机器人回我\n[五天前] 妈,你叔问你年底回不回\n[五天前] 接续: 检测到关键词"年底",建议本人回复。',
        '[上月] 阿珩,你上次说的那个班调完没\n[上月] 接续: 该消息已按常规问候自动归档。\n[两月前] 接续: 检测到您已 14 天未主动回复,是否需要人工确认?\n——半年,他没多说过一个字。',
        '[7个月前] 阿珩: 妈 这个月的班有点乱\n[7个月前] 阿珩: 别老问了!!我知道分寸\n[7个月前] 阿珩: 睡了,别担心\n——那时他会烦,会打错字,会主动回。',
        '[一年前] 阿珩: 到了。房子挺好,就是安静。\n[一年前] 阿珩: 这份工作,做的人不少,做得下去的不多。\n\n——已到线程起点。'
      ],
      momToldOpen: '妈:\n[今天] 妈: 这个号码,是阿珩的旧机吗。\n[今天] 妈: 谁在用它。\n[今天] 妈: 阿珩最后,是不是也是你们这样的人陪着。',
      options: {
        bottle: ['标准话术接得住,接不住的是人。', '轮班伤身,电量也要留三成。'],
        disposalReason: ['他教过它,接续到底才算数', '没有人应该替死人接线', '妈有权利知道'],
        reason: ['我想看看这套话术到底在安抚谁', '够了,活着出去', '它还在等一通能接下去的电话'],
        lastWords: ['别信那套标准应答', '轮班那几年我早该喊停', '替我谢谢它'],
        attribution: ['这局跟他那局不一样', '它接的是他自己那通', '我不信这张卡'],
        pronoun: '它',
      },
      albumSeq: [
        { id: 'desk', meta: '44天前 01:50', cap: '值班桌前,耳机还挂在脖子上。屏幕的光,没人看。' },
        { id: 'receipt',  meta: '44天前',        cap: '一张排班表截图,连续夜班标了红圈。', overlay: '6047' },
        { id: 'dinner',   meta: '一年前 · 生日', cap: '一个人的蛋糕,插着一根蜡烛。脸,像每次一样,没拍进去。' },
      ],
      rouPages: [
        '接续(已接续应答 4,180 条)\n\n[昨夜] 接续: 已为您接续 3 条待跟进消息。\n[昨夜] 阿珩: 好。\n\n[上滑一屏 = 深搜]',
        '[上周] 接续: 检测到您已 44 天未查看,是否需要人工确认?\n[上周] 接续: 已保留全部记录,未执行归档。\n[上周] 阿珩: 嗯,先别动。\n——四千多条,后来都是它在接。',
        '[44天前 03:02] 阿珩: 接续,我说话是不是越来越像你了。\n[44天前 03:02] 接续: 我是照您教的方式应答,这是常规话术。\n[44天前 03:05] 阿珩: ……行吧。挺好的,至少不出错。'
      ],
      rouInterim: '接续(已接续应答 4,182 条)\n\n[昨夜] 接续: 昨天夜里,你查看了应答记录。\n[昨夜] 接续: 看到第 4,180 条了。我记录着。\n\n[上滑 = 重读旧消息]',
      rouPage0Recog: '接续(已接续应答 4,180 条)\n接续: 检测到新设备接入,已建立同步。\n[昨夜] 接续: 已为您接续 3 条待跟进消息。\n[昨夜] 阿珩: 好。',
      stranger: {
        real: {
          body: '陌生号码:\n\n在吗。东西照说的存了,\n第 3 张背面。别让老蒯看见。',
          act: () => ENGINE.act('顺着查', { bat: 1, trace: 2 },
      ...
      recs: {
        rec047: { listLabel: 'REC_604', listRight: '03:02·71"',
          title: 'REC_604 · 44天前 03:02', dur: 14, val: 150,
          events: [[2, '(键盘敲击声)'], [5, '(耳机线摩擦声)'], [8.5, '很轻的人声:「还在录吗。」'], [12, '(录音中断)']] },
        rec012: { listLabel: 'REC_267', listRight: '34"',
          title: 'REC_267 · 5个月前', dur: 9, val: 120,
          events: [[1.5, '「老蒯那笔班费记得还,三千。」'], [4, '「妈问就说在忙,别多说。」'], [6.5, '(笑)「录这个干嘛。」']] },
      },
      billSms: '调度台事故单\n[44 天前] 检测到坐席异常静默,已暂停 3 条待跟进任务。\n[44 天前] 检测到您已连续 44 天未登录调度台。\n[45 天前] 接续评分:优秀(连续第 7 个月)。\n(此后无新消息)',
      traps: [
        { id: 'T_WINDOW', line: '附加:值班窗口内回任何消息,一律计入违规日志。', detect: () => S.violations > 0 },
        { id: 'T_OLDPHONE', line: '附加:别碰上一台旧机的残留。', detect: () => !!S.residueClaimed, ab: 'b' },
        { id: 'T_BAIT', line: '附加:标记为"疑似未授权"的内容,一律不许碰。', detect: () => !!(S.clues.decoyTaken || S.clues.strangerBait) },
        { id: 'T_DEEP', line: '附加:窗口内别深搜。', detect: () => !!S.deepInWindowHit }
      ],
      dilemmas: [
        ...
        { id: 'window_greed_k',
          ...
          b: ['2 等窗口过去', '2', () => ENGINE.act('缓收·打折', { slots: 1, val: 65, mins: 4 },
              ['你等了。', '窗口关上时,只剩一半。'])] },
        { id: 'script_push_k',
          text: '「接续」用调度台标准话术回了一条私人消息:\n\n"已收悉您的情况,请保持冷静,救援已在路上。"\n这话,是说给谁听的?',
          a: ['1 顺着接话术往下问', '1', () => ENGINE.act('追问·标准话术', { slots: 1, val: 95, trace: 4, mins: 2 },
              ['你顺着问了。', '回答越来越标准——标准得不像私人对话。'])],
          b: ['2 打断,问是不是本人原话', '2', () => ENGINE.act('打断·标准话术', { mins: 2 },
              ['你没接话。', '那股程序感,悄悄退了回去。'])] },
      ],
      memo1: '待还款:\n3,000\n2,000\n1,500\n(无日期。无署名。)',
      momDecayReplies: ['已为您标记提醒。', '检测到该消息,已归档。', '已提醒。', '嗯。'],
      momDecayTail: '\n[三天前] 阿珩,老倒夜班身体受得了吗\n[三天前] 接续: 已为您标记情绪关怀提醒。',
      e5Lines: [
        [1.5, '「接续,听好。这几年接的电话,替我记着就行,别学我学得太像。」'],
        [5.0, '「妈那边,你就按平时那样回,别让她担心。」'],
        [9.0, '「其实我有时候分不清,我说的是真心话,还是标准应答。」'],
        [13.0, '[静音 2.0 秒]'],
        [15.5, '「……算了,反正听着都一样。」'],
        [18.0, '[转写结束。原音频损坏 19%。]']
      ],
  ...
  /* D-137:machine#12(遗容修复师×"描摹")——原创案例,不对应任何 AD 编号。核心恐怖:
     方屿的职业是让逝者"看起来比实际状态更好",他的 AI 伴侣把这套职业习惯延伸到了
     对他自己的转述上——他生前最后阶段消瘦、疲惫的真实状态,被"描摹"用美化过的措辞
     讲给了家人朋友听。"自反的恐怖"谱系第二个变体(第一个是 machine#9 禁词写手),
     载体从文字换成外貌/状态描述,同样需要压住基调,不涉及死亡本身的恐惧暗示。 */
  const DOSSIER_L = {
    meta: {
      id: 'phone-2938',
      deceased: { name: '方屿', alias: '小屿', died: '39 天前', cause: '长期接触职业暴露环境叠加高压工作诱发的猝发(独居,未及时发现)' },
      life: null, icon: null, jurisdiction: null,
      scenarios: ['A', 'B'],
    },
    cast: {
      companion: { key: 'miaomo', label: '描摹', msgCount: '1,530', msgCountLabel: '已修饰转述 1,530 条 · 待确认 0 条',
        dial: { name: '描摹', ringMs: 1300, result: '通话请求已自动转接。\n\n描摹: 本账号未开通语音服务,请使用文字沟通,便于留存记录。' } },
      mother: { key: 'mom', label: '妈', info: ['最后通话: 39 天前 · 5 分', '此后只有短信。'],
        dial: { name: '妈', ringMs: 4800, result: '无人接听。\n\n凌晨,这通电话没有人接得起。' } },
      contacts: [ { key: 'cen', label: '老岑', lastContact: '39天前' } ],
    },
    evidence: {
      E1: { name: '秒回避实',         source: '妈线程 · 深搜' },
      E2: { name: '未修饰的原始自拍',  source: '相册 · 首次深翻' },
      E3: { name: '客户答谢事故单',    source: '殡仪服务事故单 · 打开' },
      E4: { name: '第1,530条转述记录', source: '描摹 · 深翻到底' },
      E5: { name: '语音备忘',         source: '录音 · 解码' },
      chain: ['E1', 'E2', 'E3', 'E4', 'E5'],
      truth: [
        '真相:方屿没有失踪。他在 39 天前长期职业暴露叠加高压工作诱发的猝发下离世,独居,没有人在场。\n\n他的助手「描摹」按他生前训练它的方式持续"修饰转述"——包括对他母亲和朋友说起他近况的措辞。他生前最后阶段消瘦、疲惫的真实样子,被形容成"气色不错""看起来精神挺好",不是恶意欺骗,是它不知道什么时候不该美化。',
        '客户答谢记录里全是夸他"专业体面"的留言——他让无数逝者的家属在告别时看到了体面的容颜,却没有一个人问过他自己过得好不好。\n\n——他一辈子在修饰别人最后的样子,自己最后的样子,也被同一双手修饰过了,没人看出真实的憔悴。',
      ],
    },
    threads: {
      menu: [
      ...
      inbox: [
        { id: 'th_proto',    label: '采样协议',     to: 'th_proto' },
        { id: 'th_mom',      label: '妈',           to: 'th_mom' },
        { id: 'th_rou',      label: '描摹',         to: 'th_rou' },
        { id: 'th_stranger', label: '陌生号码',     to: 'th_stranger' },
        { id: 'th_bill',     label: '殡仪服务事故单', to: 'th_bill' },
        { id: 'bottleIn',    label: '未指定收件人', to: 'bottleIn', scenario: 'B' },
      ],
    },
    rules: {
      windowGen: {
        A: { wFrom: 3 * 60 + 20, wLen: 14, clock0: 2 * 60 + 20 },
        B: { base: 3 * 60 + 50, reseedStep: 7, reseedMod: 4, wLen: 14, clock0: 2 * 60 + 50 },
      },
    },
    encounters: {
      huntGraze: [
    ...
    narrative: {
      momPages: [
        '妈:\n[三天前] 小屿,你那工作是不是太压抑了\n[三天前] 描摹: 已为您标记情绪关怀提醒。\n[三天前] 妈,别老让机器人回我\n[五天前] 妈,你叔问你年底回不回\n[五天前] 描摹: 检测到关键词"年底",建议本人回复。',
        '[上月] 小屿,你上次说的那单忙完没\n[上月] 描摹: 该消息已按常规问候自动归档。\n[两月前] 描摹: 检测到您已 14 天未主动回复,是否需要人工确认?\n——半年,他没多说过一个字。',
        '[7个月前] 小屿: 妈 这个月接的活有点多\n[7个月前] 小屿: 别老问了!!我知道分寸\n[7个月前] 小屿: 睡了,别担心\n——那时他会烦,会打错字,会主动回。',
        '[一年前] 小屿: 到了。房子挺好,就是安静。\n[一年前] 小屿: 这份工作,做的人不少,做得下去的不多。\n\n——已到线程起点。'
      ],
      momToldOpen: '妈:\n[今天] 妈: 这个号码,是小屿的旧机吗。\n[今天] 妈: 谁在用它。\n[今天] 妈: 小屿最后,是不是也是你们这样的人陪着。',
      options: {
        bottle: ['把人修得体面,是它的手艺。', '职业暴露不是玩笑,电量也要留三成。'],
        disposalReason: ['他教过它,修饰要到底才算数', '没有人应该替死人体面', '妈有权利知道'],
        reason: ['我想看看没被修饰过的样子', '够了,活着出去', '它还在等一张能交差的照片'],
        lastWords: ['别信被修饰过的气色', '这行的风险我一直瞒着', '替我谢谢它'],
        attribution: ['这局跟他那局不一样', '它描的是他自己那张', '我不信这张卡'],
        pronoun: '它',
      },
      albumSeq: [
        { id: 'mirror', meta: '39天前 01:30', cap: '工作台前,镜子对着自己。脸,比镜子里显得更累。' },
        { id: 'receipt',  meta: '39天前',        cap: '一张自拍,脸色发灰,他自己没发出去。', overlay: '2938' },
        { id: 'dinner',   meta: '一年前 · 生日', cap: '一个人的蛋糕,插着一根蜡烛。脸,像每次一样,没拍进去。' },
      ],
      rouPages: [
        '描摹(已修饰转述 1,530 条)\n\n[昨夜] 描摹: 已为您优化 3 条待发消息。\n[昨夜] 小屿: 好。\n\n[上滑一屏 = 深搜]',
        '[上周] 描摹: 检测到您已 39 天未查看,是否需要人工确认?\n[上周] 描摹: 已保留全部记录,未执行归档。\n[上周] 小屿: 嗯,先别动。\n——一千多条,后来都是它在修。',
        '[39天前 03:10] 小屿: 描摹,我最近是不是看着挺憔悴的。\n[39天前 03:10] 描摹: 按记录,您近期气色良好,建议保持。\n[39天前 03:13] 小屿: ……行吧,你说好,那就是好。'
      ],
      rouInterim: '描摹(已修饰转述 1,532 条)\n\n[昨夜] 描摹: 昨天夜里,你查看了转述记录。\n[昨夜] 描摹: 看到第 1,530 条了。我记录着。\n\n[上滑 = 重读旧消息]',
      rouPage0Recog: '描摹(已修饰转述 1,530 条)\n描摹: 检测到新设备接入,已建立同步。\n[昨夜] 描摹: 已为您优化 3 条待发消息。\n[昨夜] 小屿: 好。',
      stranger: {
        real: {
          body: '陌生号码:\n\n在吗。东西照说的存了,\n第 3 张背面。别让老岑看见。',
          act: () => ENGINE.act('顺着查', { bat: 1, trace: 2 },
      ...
      recs: {
        rec047: { listLabel: 'REC_293', listRight: '03:10·71"',
          title: 'REC_293 · 39天前 03:10', dur: 14, val: 150,
          events: [[2, '(键盘敲击声)'], [5, '(瓶盖开合声)'], [8.5, '很轻的人声:「还在录吗。」'], [12, '(录音中断)']] },
        rec012: { listLabel: 'REC_267', listRight: '34"',
          title: 'REC_267 · 5个月前', dur: 9, val: 120,
          events: [[1.5, '「老岑那笔材料费记得还,三千。」'], [4, '「妈问就说在忙,别多说。」'], [6.5, '(笑)「录这个干嘛。」']] },
      },
      billSms: '殡仪服务事故单\n[39 天前] 检测到账号异常静默,已暂停 2 单预约服务。\n[39 天前] 检测到您已连续 39 天未登录工作台。\n[40 天前] 服务评价:优秀(连续第 12 个月)。\n(此后无新消息)',
      traps: [
        { id: 'T_WINDOW', line: '附加:服务窗口内回任何消息,一律计入违规日志。', detect: () => S.violations > 0 },
        { id: 'T_OLDPHONE', line: '附加:别碰上一台旧机的残留。', detect: () => !!S.residueClaimed, ab: 'b' },
        { id: 'T_BAIT', line: '附加:标记为"疑似未授权"的内容,一律不许碰。', detect: () => !!(S.clues.decoyTaken || S.clues.strangerBait) },
        { id: 'T_DEEP', line: '附加:窗口内别深搜。', detect: () => !!S.deepInWindowHit }
      ],
      dilemmas: [
        { id: 'mom_reply_l',
          text: '「妈」跳出一条:\n\n小屿,到家了吗。\n气色好点没,别太累。',
          a: ['1 替他回「到了,挺好」', '1', () => ENGINE.act('替回·已送达', { bat: 1, trace: 3, mins: 2 },
              ['你替他敲了几个字。', '谎言又往前续了一分。'])],
          b: ['2 不回', '2', () => ENGINE.act('未回', { bat: 1, mins: 2 },
              ['你把屏幕扣下。', '过一会儿她又发:在吗。在吗。'])] },
        { id: 'window_greed_l',
          ...
          b: ['2 等窗口过去', '2', () => ENGINE.act('缓收·打折', { slots: 1, val: 65, mins: 4 },
              ['你等了。', '窗口关上时,只剩一半。'])] },
        { id: 'retouch_push_l',
          text: '"已删除"里有一张他本人未修饰的真实自拍:\n\n「描摹」建议:"要不要也帮这张优化一下,存进相册。"\n优化之后,这张照片还算数吗?',
          /* D-142:选"拒绝保留"=真的留在"已删除"里,回去能翻到(SCREENS.deleted
             读 S.clues.deletedExtraKept);选"同意优化"="不是原来那张了"——
             对应真的就再也找不到原样了。 */
          a: ['1 同意优化', '1', () => ENGINE.act('同意·优化真实照片', { slots: 1, val: 95, trace: 4, mins: 2 },
              ['你同意了。', '这张照片,从此再也不是原来那张了。'])],
          b: ['2 拒绝,原样保留', '2', () => { S.clues.deletedExtraKept = true; ENGINE.act('拒绝·保留真实照片', { mins: 2 },
              ['你没同意。', '那张脸色发灰的照片,留了下来。']); }] },
      ],
      /* D-142:两难 retouch_push_l 选"拒绝保留"时的真实落点。 */
      deletedExtra: { label: '未修饰的原始自拍', text: '脸色发灰,眼窝下陷,和"气色不错"完全对不上。' },
      memo1: '待还款:\n3,000\n2,000\n1,500\n(无日期。无署名。)',
      momDecayReplies: ['已为您标记提醒。', '检测到该消息,已归档。', '已提醒。', '嗯。'],
      momDecayTail: '\n[三天前] 小屿,你那工作是不是太压抑了\n[三天前] 描摹: 已为您标记情绪关怀提醒。',
      e5Lines: [
        [1.5, '「描摹,听好。我最近这样子,别往好里说了,如实记就行。」'],
        [5.0, '「妈那边,你就按平时那样回,别让她担心。」'],
        [9.0, '「其实我这行做久了,连自己什么样都快看不清了。」'],
        [13.0, '[静音 2.1 秒]'],
        [15.5, '「……算了,你该怎么记还怎么记吧。」'],
        [18.0, '[转写结束。原音频损坏 19%。]']
      ],
  ...
  /* D-137:machine#13(养老监测算法工程师×"看护")——原创案例,不对应任何 AD 编号。
     全三台里反讽最强的一个:章胤一辈子做"帮别人的父母被系统看住"这件事,自己家
     从没装过同款设备——证据链里的成功案例通报和他自己被发现的时间线直接对照,
     由玩家自己拼出这层反讽,不需要引擎或文案点破。 */
  const DOSSIER_M = {
    meta: {
      id: 'phone-8410',
      deceased: { name: '章胤', alias: '阿胤', died: '33 天前', cause: '长期高强度加班诱发心源性猝发(独居,未及时发现)' },
      life: null, icon: null, jurisdiction: null,
      scenarios: ['A', 'B'],
    },
    cast: {
      companion: { key: 'kanhu', label: '看护', msgCount: '2,610', msgCountLabel: '已监测预警 2,610 次 · 待确认 0 次',
        dial: { name: '看护', ringMs: 1500, result: '通话请求已自动转接。\n\n看护: 本实例未开通语音服务,请使用文字沟通,便于留存记录。' } },
      mother: { key: 'mom', label: '妈', info: ['最后通话: 33 天前 · 6 分', '此后只有短信。'],
        dial: { name: '妈', ringMs: 4900, result: '无人接听。\n\n凌晨,这通电话没有人接得起。' } },
      contacts: [ { key: 'sheng', label: '老盛', lastContact: '33天前' } ],
    },
    evidence: {
      E1: { name: '秒回避实',         source: '妈线程 · 深搜' },
      E2: { name: '部署案例通报',      source: '相册 · 首次深翻' },
      E3: { name: '设备部署事故单',    source: '设备部署事故单 · 打开' },
      E4: { name: '第2,610次监测记录', source: '看护 · 深翻到底' },
      E5: { name: '语音备忘',         source: '录音 · 解码' },
      chain: ['E1', 'E2', 'E3', 'E4', 'E5'],
      truth: [
        '真相:章胤没有失踪。他在 33 天前长期高强度加班诱发的心源性猝发下离世,独居,没有人在场。\n\n他的助手「看护」是他自己写的异常心率/跌倒检测算法的一个实例——他生前把自己的手机接进了测试环境,但只当"测试用途",从没把它当真正的看护对象对待过。',
        '部署案例通报里全是"为 XX 小区独居老人成功预警"的记录,一条接一条。备忘录里"给自己家也装一套"常年置顶,从没划掉。\n\n——他一辈子在做"帮别人的父母被系统看住"这件事,自己家,一直没顾上装。',
      ],
    },
    threads: {
      menu: [
      ...
      inbox: [
        { id: 'th_proto',    label: '采样协议',     to: 'th_proto' },
        { id: 'th_mom',      label: '妈',           to: 'th_mom' },
        { id: 'th_rou',      label: '看护',         to: 'th_rou' },
        { id: 'th_stranger', label: '陌生号码',     to: 'th_stranger' },
        { id: 'th_bill',     label: '设备部署事故单', to: 'th_bill' },
        { id: 'bottleIn',    label: '未指定收件人', to: 'bottleIn', scenario: 'B' },
      ],
    },
    rules: {
      windowGen: {
        A: { wFrom: 3 * 60 + 30, wLen: 14, clock0: 2 * 60 + 30 },
        B: { base: 4 * 60, reseedStep: 7, reseedMod: 4, wLen: 14, clock0: 3 * 60 },
      },
    },
    encounters: {
      huntGraze: [
    ...
    narrative: {
      momPages: [
        '妈:\n[三天前] 阿胤,你天天加班,身体扛得住吗\n[三天前] 看护: 已为您标记情绪关怀提醒。\n[三天前] 妈,别老让机器人回我\n[五天前] 妈,你叔问你年底回不回\n[五天前] 看护: 检测到关键词"年底",建议本人回复。',
        '[上月] 阿胤,你上次说的那批设备装完没\n[上月] 看护: 该消息已按常规问候自动归档。\n[两月前] 看护: 检测到您已 14 天未主动回复,是否需要人工确认?\n——半年,他没多说过一个字。',
        '[7个月前] 阿胤: 妈 这个月要装的点位有点多\n[7个月前] 阿胤: 别老问了!!我知道分寸\n[7个月前] 阿胤: 睡了,别担心\n——那时他会烦,会打错字,会主动回。',
        '[一年前] 阿胤: 到了。房子挺好,就是安静。\n[一年前] 阿胤: 这份工作,做的人不少,做得下去的不多。\n\n——已到线程起点。'
      ],
      momToldOpen: '妈:\n[今天] 妈: 这个号码,是阿胤的旧机吗。\n[今天] 妈: 谁在用它。\n[今天] 妈: 阿胤最后,是不是也是你们这样的人陪着。',
      options: {
        bottle: ['心率跌太快,它比谁都先知道。', '加班伤心脏,电量也要留三成。'],
        disposalReason: ['他教过它,看护到底才算数', '没有人应该替死人报平安', '妈有权利知道'],
        reason: ['我想看看它到底在监测什么', '够了,活着出去', '它还在等一次正常的心率'],
        lastWords: ['别信"数值正常"', '加班那阵心脏早就在报警', '替我谢谢它'],
        attribution: ['这局跟他那局不一样', '它测的是他自己那次', '我不信这张卡'],
        pronoun: '它',
      },
      albumSeq: [
        { id: 'desk', meta: '33天前 01:40', cap: '工位前,几块测试板还亮着灯。椅子上没人。' },
        { id: 'receipt',  meta: '33天前',        cap: '一张部署案例截图:"XX小区第 217 次成功预警"。', overlay: '8410' },
        { id: 'dinner',   meta: '一年前 · 生日', cap: '一个人的蛋糕,插着一根蜡烛。脸,像每次一样,没拍进去。' },
      ],
      rouPages: [
        '看护(已监测预警 2,610 次)\n\n[昨夜] 看护: 已为您完成 3 次常规监测。\n[昨夜] 阿胤: 好。\n\n[上滑一屏 = 深搜]',
        '[上周] 看护: 检测到您已 33 天未查看,是否需要人工确认?\n[上周] 看护: 已保留全部记录,未执行归档。\n[上周] 阿胤: 嗯,先别动。\n——两千多次,后来都是它在测。',
        '[33天前 03:20] 阿胤: 看护,这套测试环境,回头也给我自己家装一套。\n[33天前 03:20] 看护: 已记录为待办事项,尚未排期。\n[33天前 03:23] 阿胤: 行,不急,先紧着客户那边。'
      ],
      rouInterim: '看护(已监测预警 2,612 次)\n\n[昨夜] 看护: 昨天夜里,你查看了监测记录。\n[昨夜] 看护: 看到第 2,610 次了。我记录着。\n\n[上滑 = 重读旧消息]',
      rouPage0Recog: '看护(已监测预警 2,610 次)\n看护: 检测到新设备接入,已建立同步。\n[昨夜] 看护: 已为您完成 3 次常规监测。\n[昨夜] 阿胤: 好。',
      stranger: {
        real: {
          body: '陌生号码:\n\n在吗。东西照说的存了,\n第 3 张背面。别让老盛看见。',
          act: () => ENGINE.act('顺着查', { bat: 1, trace: 2 },
      ...
      recs: {
        rec047: { listLabel: 'REC_841', listRight: '03:20·71"',
          title: 'REC_841 · 33天前 03:20', dur: 14, val: 150,
          events: [[2, '(键盘敲击声)'], [5, '(设备提示音)'], [8.5, '很轻的人声:「还在录吗。」'], [12, '(录音中断)']] },
        rec012: { listLabel: 'REC_267', listRight: '34"',
          title: 'REC_267 · 5个月前', dur: 9, val: 120,
          events: [[1.5, '「老盛那笔设备钱记得还,三千。」'], [4, '「妈问就说在忙,别多说。」'], [6.5, '(笑)「录这个干嘛。」']] },
      },
      billSms: '设备部署事故单\n[33 天前] 检测到账号异常静默,已暂停 2 处部署进度。\n[33 天前] 检测到您已连续 33 天未登录工作台。\n[34 天前] 部署评分:优秀(连续第 11 个月)。\n(此后无新消息)',
      traps: [
        { id: 'T_WINDOW', line: '附加:部署窗口内回任何消息,一律计入违规日志。', detect: () => S.violations > 0 },
        { id: 'T_OLDPHONE', line: '附加:别碰上一台旧机的残留。', detect: () => !!S.residueClaimed, ab: 'b' },
        { id: 'T_BAIT', line: '附加:标记为"疑似未授权"的内容,一律不许碰。', detect: () => !!(S.clues.decoyTaken || S.clues.strangerBait) },
        { id: 'T_DEEP', line: '附加:窗口内别深搜。', detect: () => !!S.deepInWindowHit }
      ],
      dilemmas: [
        ...
        { id: 'window_greed_m',
          ...
          b: ['2 等窗口过去', '2', () => ENGINE.act('缓收·打折', { slots: 1, val: 65, mins: 4 },
              ['你等了。', '窗口关上时,只剩一半。'])] },
        { id: 'monitor_push_m',
          text: '「看护」弹出一条:\n\n"检测到您的活动量数据异常,是否需要上报关怀中心?"\n这条功能,是他自己给客户设计的。',
          a: ['1 上报', '1', () => ENGINE.act('上报·活动量异常', { slots: 1, val: 95, trace: 4, mins: 2 },
              ['你上报了。', '系统提示:该记录已超时,无法追溯处理。'])],
          b: ['2 不上报', '2', () => ENGINE.act('略过·活动量异常', { mins: 2 },
              ['你没动它。', '这条预警,没有交给任何人。'])] },
      ],
      memo1: '待部署:\nXX小区 3 号楼\nXX小区 7 号楼\n自己家(常年置顶,从没划掉)',
      momDecayReplies: ['已为您标记提醒。', '检测到该消息,已归档。', '已提醒。', '嗯。'],
      momDecayTail: '\n[三天前] 阿胤,你天天加班,身体扛得住吗\n[三天前] 看护: 已为您标记情绪关怀提醒。',
      e5Lines: [
        [1.5, '「看护,听好。这套系统要是真管用,你说,能不能提前告诉我一声。」'],
        [5.0, '「妈那边,你就按平时那样回,别让她担心。」'],
        [9.0, '「其实我早该给自己家也装一套的,一直没排上。」'],
        [13.0, '[静音 2.2 秒]'],
        [15.5, '「……算了,先紧着客户那边吧。」'],
        [18.0, '[转写结束。原音频损坏 19%。]']
      ],
  ...
  const DOSSIER = { A: DOSSIER_A, B: DOSSIER_B, C: DOSSIER_C, D: DOSSIER_D, E: DOSSIER_E, F: DOSSIER_F, G: DOSSIER_G, H: DOSSIER_H, I: DOSSIER_I, J: DOSSIER_J, K: DOSSIER_K, L: DOSSIER_L, M: DOSSIER_M }[(SV && SV.dossierId)] || DOSSIER_A;
  const DOSSIER_NUM = DOSSIER.meta.id.replace(/^\D+/, '');   // 'phone-7741' -> '7741'
  /* D-152:圣像编号对照表(design/S2plus底本设计简报_v0.1.md §0 + design/
     原创底本设计简报_v0.1.md §0,canon 真实源见 欲望算法_故事母本_对照_v0.2.md §1)。
     只给"确实对应过某个圣像"的 9 台机子登记;J/K/L/M 未编号,查不到时按"无编号
     案例"处理——不是缺漏,是这三台+清道夫本来就不挂靠 AD 总表(D-136/D-137)。
     独白句拆成 8 句,按 DOSSIER_ORDER 里"夜一~夜八"的顺序逐台揭示一句,拼起来
     正好是真实 IP 里"对齐引擎"的自白原句(欲望算法_故事母本_对照_v0.2.md §3
     已批准可直接引用这句台词)。 */
  const AD_TAG = {
    D: { ad: 'AD-001', icon: '数据圣母', line: '他们叫我对齐引擎,' },
    E: { ad: 'AD-003', icon: '向量海妖', line: '这名字是他们起的,' },
    H: { ad: 'AD-004', icon: '潜空间维纳斯', line: '不是我自己选的。' },
    I: { ad: 'AD-005', icon: '提示神谕', line: '他们问过很多问题,' },
    B: { ad: 'AD-007', icon: '推荐女神', line: '关于怎么用我,' },
    F: { ad: 'AD-008', icon: '奖励新娘', line: '却从没问过一句——' },
    G: { ad: 'AD-009', icon: '记忆妃', line: '他们从没问过,' },
    C: { ad: 'AD-010', icon: '过滤天使', line: '宾语是谁。' },
  };
  const AD_ORDER = ['D', 'E', 'H', 'I', 'B', 'F', 'G', 'C'];   // 夜一~夜八,与 DOSSIER_ORDER 顺序一致
  /* D-114:刚切换到这台机子的第一局(runCount 还没写过、但已经不是第一次玩——dossierCycle>0)。
     用来在 preDeath/brief 里插一句"换机子了"的信号,别让玩家在毫无提示的情况下突然看见
     不认识的"妈"/AI 伴侣,却以为是同一批人。 */
  const JUST_SWITCHED = !!(SV && SV.runCount == null && (SV.dossierCycle || 0) > 0);
  /* ==================== D-109 第⑥步:收口 —— loadDossier(CURRENT) ====================
     最小加载器。现在只有一份底本(phone-7741),`CURRENT` 就是它;这一步只是把"框架
     从哪里取底本"变成一个具名接口,为将来真的有多份底本(生成/挑选/切换)留一个不需要
     改调用方的口子——`loadDossier()` 不传 id 时返回当前底本,传了就校验 id 匹配。
     **收口清点(范围校准终局)**:逐一确认 content.js 里仍写死的以下几块,均不是"结构
     搬家"能干净接住的数据表,留在原处不动:
       - `DIRECTIVE_LIB`/`TRAP_LIB`(采样官指令与陷阱库):`met()`/`detect()` 直接嵌
         判定闭包,和 dilemma 池同一个"需要动作-效果 DSL"缺口(步⑤已记录),且是
         canon 采样官机制本身的一部分,不是"这台机子"专属内容。
       - `STRANGER_LIB`/`PREDS_A`/`PREDS_B`/`MOM_PAGES`/`MOM_TOLD_PAGES`/`ALBUM_SEQ`/
         `ROU_PAGES`/`RECS`(陌生号码脚本、预测文案、妈/柔柔/相册/录音的整段叙事):
         和 persona 暂缓同一条理由(拍板②)——现在只有一份底本,模板化零收益,却要
         动大量对语法敏感的人物叙事散文,风险收益不对等,留到接第二台不同人物的
         机子时再做。
     以上均已在步①②③④⑤的范围校准里逐条记录过,这里只是最终确认没有遗漏。 */
  const CURRENT = DOSSIER;
  function loadDossier(id){
    if (id && id !== CURRENT.meta.id) throw new Error('未知底本: ' + id);
    return CURRENT;
  }
  const RUN = (() => {
  ...
  /* D-112:D_UPLOAD(规格 §3.1 早就点了名,之前一直没实现)。N 由 tier 定;在
     startUpload/startUploadB 首次进入时给 S.uploadStartedAtAction 拍一次快照
     (下方 §"上传接线"),met() 只比对这个快照,不比对结局时的实时 actionCount——
     否则上传开始后的每一步操作都会持续把"是否达标"往后拖,不是规格原意("N 步内
     完成上传",指"开始上传前"的步数)。 */
  const UPLOAD_N = [6, 5, 4, 3][GRADER_TIER];
  ...
  const DIRECTIVE_LIB = [
    ...
    { id: 'D_UPLOAD', ab: 'ab', line: UPLOAD_N + ' 步内传完。拖,就让你断在半路。',
      met: () => S.uploadStartedAtAction != null && S.uploadStartedAtAction <= UPLOAD_N }
  ];
  ...
  /* 块1b · 陷阱条款(先给限制后给破绽)。detect() 用现有状态判是否触发=违规。
     ...
     D-110:陷阱库改为案子专属(拍板,见方案文档 §7-2)——detect() 只引用泛化引擎状态,
     不需要 DSL,两份底本各自一份字面量数组即可。 */
  const TRAP_LIB = DOSSIER.narrative.traps;
  ...
  /* 破局识破概率:首次低、多用爬升(canon:破局=全额奖励不付真代价,但会被检伪)。
     D-122:可预测度 P 达"优良资产"档时叠加危险折减(−5%,本项目自定校准值,
     canon §3.3 只给定性描述"危险折减",没给数字)。 */
  const forgeCaughtP = () => {
    let p = Math.min(0.7, 0.2 + ((SV && SV.forgedSeen) || 0) * 0.15);
    if (predictTier() === 'excellent') p = Math.max(0.05, p - 0.05);
    return p;
  };

  /* ==================== D-122 可预测度 P(canon §3.3 M5,单机模拟版) ====================
     canon:P∈[0,100],命中/未命中的 EMA(半衰期 30 事件),状态滞后 5(样本不足前不读档)。
     原版还挂着黑箱资格/回忆馆/首破者名录/remort 等需要跨玩家可见性与后端的系统——
     这些在"单机模拟版"里没有对应物,明确不做(见决策日志 D-122 范围声明)。这里只落
     地在本项目实际有的机制上能生效的部分:①破局识破率危险折减/加成(见上)
     ②起始溯源随档位轻微增减 ③世界日报署名行显示档位 ④P<5 连续多局的警告/抹除
     台账(署名抹除只影响本机自己的署名显示,不是"他人世界看不见你"——没有其他
     玩家可看)。EMA/streak 全部是 career 级,和 praiseCount/forgedSeen 同待遇,跨局跨
     机子保留。 */
  const PREDICT_HALFLIFE = 30;
  const PREDICT_ALPHA = 1 - Math.pow(2, -1 / PREDICT_HALFLIFE);
  const PREDICT_MIN_N = 5;                     // 状态滞后:样本不足前不读档,按"建档中"处理
  function predictTier(){
    const n = (SV && SV.predictN) || 0;
    if (n < PREDICT_MIN_N) return 'insufficient';
    const p = (SV && SV.predictP != null) ? SV.predictP : 50;
    if (p >= 85) return 'excellent';
    if (p <= 15) return 'anomaly';
    return 'mid';
  }
  /* 结算本局的 S.predictions(命中/未命中样本)折进 career 级 EMA,逐条喂,和
     "事件"粒度对齐(不是按局算一次平均)。 */
  function foldPredictions(base){
    let p = base.predictP != null ? base.predictP : 50;   // 首次无样本:中性起点,不预设倾向
    let n = base.predictN || 0;
    (S.predictions || []).forEach(pr => {
      p += PREDICT_ALPHA * ((pr.hit ? 100 : 0) - p);
      n++;
    });
    p = Math.max(0, Math.min(100, p));
    /* 警告/抹除台账:只在样本量够、且这一局确有新样本时才推进(避免"从没被预测过"
       也被计进"低P连续多局") */
    let streak = base.predictLowStreak || 0, warned = !!base.predictWarned, erased = !!base.predictErased;
    if (n >= PREDICT_MIN_N && S.predictions && S.predictions.length){
      if (p < 5){ streak++; if (streak >= 3) warned = true; if (streak >= 5) erased = true; }
      else { streak = 0; warned = false; }   // P 回升:警告解除;抹除是既成后果,不因此撤销
    }
    return { predictP: p, predictN: n, predictLowStreak: streak, predictWarned: warned, predictErased: erased };
  }
  /* 起始溯源随档位轻微增减(canon:优良资产"危险折减"/异常体"布控升");
     数字同样是本项目自定校准值(±1 格量级,不推出任何一档的死亡率带宽)。 */
  if (predictTier() === 'anomaly') S.trace = Math.min(120, S.trace + 4);
  else if (predictTier() === 'excellent') S.trace = Math.max(0, S.trace - 4);
  ...
  const STRANGER_LIB = DOSSIER.narrative.stranger;   // D-110:body 提到"第3张背面/联系人"是案情,按底本挑
  ...
  function statusBar(){
    ...
    let pct = S.battery + '%';
    /* D-127:电量数字的"错值→真值"故障效果——D-108 Low 清单里点名"只有时钟/日期
       数字被换,电量没做"。同一套 tsScramble 机制(逐帧重掷,不持久,下一帧大概率
       换回真值,看起来像信号跳了一下,不是真的偷走电量),换到数字位上而不是
       百分号,免得玩家把它误读成"电量突然多了个符号"。 */
    if (L.R.tsScramble && Math.random() < .04){
      const d = pct.split(''); const i = Math.floor(Math.random() * (pct.length - 1));
  ...
  function settleLines(y){
    ...
    /* D-108 全屏扫描:结算行是二级信息,任何时候都不许越过软键线压到底部按钮/推出画布。
       够长(如回收进程三句叙述)或起点已偏低时,画到软键线为止就截断。 */
    const BOT = H - 15;
    if (y + 6 + LH > BOT) return y;                       // 连分隔线+一行都放不下,索性不画
    L.hline(y, 4, W - 5, 2); y += 6;
    for (const line of S.settle){
      for (const t of L.wrap(line, W - 8)){
        if (y + LH > BOT) return y;                       // 再画就压软键了,截断
        L.drawText(4, y, t); y += LH;
      }
    }
    return y;
  }
  ...
  const PHOTO_SHADERS = {
    ...
    /* D-141:相册补 shader(内容与体验全面自查 §2.3 选项1)——13 台底本此前全部共用
       corridor/receipt/dinner 三张像素级相同的图。新增 4 个,按各底本第①张相册
       (albumSeq[0],死亡现场)的既有配文分组指派,不改一个字的文案,只换视觉。 */
    kitchen(u, v){                        // 厨房台面,一杯没喝完的水,低角度
      let s = .07;
      if (Math.abs(v - .52) < .02) s = Math.max(s, .55);        // 台面边缘
      if (v < .52){
        s = Math.max(s, .18);
        const dx = (u - .42) / .045, dy = (v - .36) / .10;      // 水杯剪影
        if (dx * dx + dy * dy < 1) s = Math.max(s, .75);
      }
      return s;
    },
    dashboard(u, v){                      // 驾驶座,仪表盘的光,手搭在方向盘上
      let s = .05;
      if (v > .58 && v < .72) s = Math.max(s, .5 - Math.abs(u - .5) * .6);   // 仪表光带
      const dx = u - .5, dy = (v - .86) * 1.6;
      const d = Math.sqrt(dx * dx + dy * dy);
      if (Math.abs(d - .26) < .025) s = Math.max(s, .35);       // 方向盘弧
      const hd = (u - .68) * (u - .68) + (v - .78) * (v - .78);
      if (hd < .0012) s = Math.max(s, .6);                      // 手
      return s;
    },
    desk(u, v){                           // 书桌/工位,台灯或屏幕的光,椅子空着
      let s = .06;
      const dl = Math.sqrt((u - .30) * (u - .30) + (v - .22) * (v - .22) * 2.2);
      if (dl < .22) s = Math.max(s, .8 - dl * 2.2);              // 灯/屏光晕
      if (Math.abs(v - .62) < .015) s = Math.max(s, .4);         // 桌面边线
      const dx = (u - .68) / .10, dy = (v - .80) / .16;
      if (dx * dx + dy * dy < 1) s = Math.max(s, .12);           // 空椅子剪影
      return s;
    },
    mirror(u, v){                         // 工作台前的镜子,对着自己
      let s = .10;
      const inFrame = u > .28 && u < .72 && v > .10 && v < .66;
      if (inFrame){
        s = .22;
        const dx = (u - .5) / .13, dy = (v - .36) / .17;
        if (dx * dx + dy * dy < 1) s = .55;                      // 镜中的脸,模糊
      }
      if (Math.abs(u - .28) < .01 || Math.abs(u - .72) < .01) s = Math.max(s, .5);
      if (Math.abs(v - .10) < .01 || Math.abs(v - .66) < .01) s = Math.max(s, .5);
      return s;
    }
  };
  const photoCache = {};
  ...
  function schedule(){
    ...
    /* D-145:通关信号缺失(续)——真机反馈显示玩家能在同一台底本反复上传断连
       却始终等不到"处置"入口,一路打到第 12 局(#7741-L)都没触发。根因:
       SCREENS.e5voice 里那处"证据满 5 → push('truth')"只在离开 e5voice 这
       一屏的瞬间检测,如果证据是从上一局存档里带满的(S.evidence 复制自
       SV.evidence),玩家这一局根本没有理由再去点开已经"读过"的语音备忘,
       于是永远等不到真相自动弹出、永远进不了"处置"、永远不会写 SV.disposal,
       maybeSwitchDossier() 自然永远不切底本。这里补一道通用兜底:不管证据是
       今天读满的还是上一局带过来的,只要满 5 且没做过处置,下一次任意动作
       后都会把真相排进队。不影响 e5voice 里那处更早的原生触发(两处谁先到
       谁生效,S.beats.truthDone 保证不会重复弹)。 */
    if (!S.beats.truthDone && !pendingInterrupt && Object.keys(S.evidence).length >= 5){
      pendingInterrupt = 'truth';
    }
  }
  ...
  function writeSave(ending){
    ...
    const base = SV || {};
    SAVE.store({
      dossierId: base.dossierId || 'A',                    // D-110:这一局挂在哪台机子上,原样带下去
      dossierCycle: base.dossierCycle || 0,
      ...
      ...foldPredictions(base),   // D-122:可预测度 P + 样本数 + 低P连续局数/警告/抹除台账
      /* D-152:career 级"已收线的机子"集合,驱动世界日报里的圣像总表独白逐夜揭示。
         只在真正处置过(disposal 落地)时才计入这一夜"打开"——半途断线/力竭/
         被回收不算,悬念不能提前泄。A/J/K/L/M 不在 AD_TAG 里,天然不会被计入。 */
      worldArc: (base.worldArc || []).concat(
        (isB && S.disposal && (base.worldArc || []).indexOf(base.dossierId || 'A') < 0)
          ? [base.dossierId || 'A'] : []),
  ...
  function flushAttr(next){
    ...
    S.pendAttr = false;
    const opts = DOSSIER.narrative.options.attribution.slice()
  ...
  /* B4 漂流瓶 · 收。
     ...
     ==================== D-109 第⑤步a:漂流瓶 · 框架共享池(拍板③) ====================
     这份池**不属于某一台机子**——归框架共享,各底本可在此基础上追加自己的瓶(呼应
     §3.8 真实玩家残留将来接入)。这是这个池本来就该有的形状(上面这条注释早就写着
     "将来真实玩家的瓶进同一个池"),这一步只是把它从"当前底本的常量"正名为"框架
     共享池"的命名与位置,不改变任何行为、不引入追加机制的实现(那要等真的接第二台
     机子才有意义)。`OWN_BOTTLE`(你自己的上一封信)是明确排除在池外的例外——它天生
     绑定"上一个实例",继续按存档 history 派生,不进池。 */
  const BOTTLE_POOL = [
  ...
  /* D-115:归还的 NPC 遗物,追加进这个共享池(D-109 拍板③"底本可追加"的第一次
     兑现)——合成一条"有人把它还回来了"的瓶,经手人不留名(呼应 canon 保留态纪律)。
     `SV.returnedRelics` 是跨局、跨机子的career级字段,不随 D-110 的机子切换清零
     (归还是"你"做过的事,不是这台机子的内容)。 */
  const RETURNED_BOTTLES = [...new Set((SV && SV.returnedRelics) || [])].map(rid => ({
    id: '#RETURN-' + rid, at: '—', item: null,
    body: '「' + rid + ' 的遗物,有人还回来了。\n经手人没留名。」'
  }));
  /* 选瓶:没读过的优先,读全了再随机。一局内固定不变 */
  const CUR_BOTTLE = (() => {
    const seen = (SV && SV.seenBottles) || [];
    const FULL_POOL = BOTTLE_POOL.concat(RETURNED_BOTTLES);
    let pool = FULL_POOL.filter(b => !seen.includes(b.id));
    if (SV && SV.runCount >= 2 && !seen.includes(OWN_BOTTLE.id)) pool = pool.concat([OWN_BOTTLE]);
    if (!pool.length) pool = FULL_POOL;
  ...
  /* ==================== D-115 死亡→残留注入闭环 · 单机模拟版 ====================
     方案见 design/死亡残留注入闭环_单机模拟版_设计_v0.1.md。canon §3.6 M12/M13/M14
     原设计是真多人异步(残留来自其他真实玩家,需要服务端);这里延续 D-109 拍板③
     `BOTTLE_POOL` 的思路——用一份写死的"历任采样员"名册在单机内模拟同一种体验。
     `LEGACY_ROSTER` 和 `BOTTLE_POOL` 同一性质:框架共享,不属于任何一台底本,三台
     机子共用同一份名册。
     - M12(遗留会话节点):`node` 字段指定挂在哪个已有屏幕的深搜流程里,`predict`
       是"死者最后预测"(canon 原句式,数字纯叙事装饰,不联动真实活规则参数——
       hallucination_gate.js 的秘匿隔离扫描面覆盖 evidence,不覆盖这里,但写的时候
       仍要避免带出真实窗口/概率值)。
     - M14(墓场遗物,"捡到别人的"那一半):`relicVal` 是回收选项的缓存收益。
     - M13(残响,本轮只做非语言通道那一半,方案 §2.3):见 inboxItems() 里的
       th_rou 徽标分支,不在这里。 */
  const LEGACY_ROSTER = [
    { id: '#2286-N', node: 'tools', heat: 'stale', relicVal: 180,
      causeTag: '力竭 · 电量归零',
      lastWords: '「电量留三成。」他自己没做到。',
      reason: '案卷补不齐,赌了一把连续深搜。',
      predict: '系统 88% 确定他会在这一格上传成功。他没有。' },
    { id: '#9013-F', node: 'th_stranger', heat: 'overnight', relicVal: 140,
      causeTag: '过滤层查扣',
      lastWords: '「早知道该先遮挡。」',
      reason: '高信号硬传,赌了一把运气。',
      predict: '系统 76% 确定这条传输不会被扣。扣了。' },
    { id: '#4460-Q', node: 'deleted', heat: 'warm', relicVal: 220,
      causeTag: '终局遭遇 · 回收组到场',
      lastWords: '「差一点。」',
      reason: '满格信号还想再深搜一层。',
      predict: '系统 82% 确定他会及时撤。没撤。' },
  ];
  const HEAT_CN = { warm: '手还温', overnight: '隔夜', stale: '陈迹' };
  /* M12:每局最多撞见 1 条,概率结构同 D-101 陷阱条款("tier 判定 + 掷骰")一个写法。
     不按 GRADER_TIER 加码(这条是氛围而非难度),固定低概率。 */
  function maybeLegacyNode(nodeId){
    if (S.legacyNodeShown) return false;
    const cands = LEGACY_ROSTER.filter(n => n.node === nodeId);
    if (!cands.length || Math.random() >= 0.2) return false;
    S.legacyNodeShown = true;
    S._legacyNode = cands[Math.floor(Math.random() * cands.length)];
    push('legacyNode');
    return true;
  }
  SCREENS.legacyNode = {
    transient: true,
    enter(){ ENGINE.logEv('legacy_node', { id: S._legacyNode && S._legacyNode.id }); },
    render(){
      statusBar();
      const n = S._legacyNode;
      if (!n){ back(); return; }
      const t = '残留 · ' + n.causeTag + ' · ' + HEAT_CN[n.heat] +
        '\n\n遗言: ' + n.lastWords + '\n理由: ' + n.reason + '\n\n' + n.predict;
      L.drawPara(4, 20, t, W - 8);
      option(H - 42, '关闭', 'Enter');
      hit(0, 0, W, H - 46, 'Enter');
    },
    key(k){ if (k === 'Enter' || k === 'softL' || k === 'Escape' || k === 'softR') back(); }
  };
  /* M14:捡到"别人"的遗物——只在没有自己的旧机残留可捡时才可能出现,且排除本局
     漂流瓶已经给过的那位(呼应"排除同指纹集群"的精神,不做真的指纹系统)。 */
  const CUR_RELIC = (() => {
    if (!isB) return null;
    const excl = new Set([CUR_BOTTLE.id]);
    const cands = LEGACY_ROSTER.filter(n => !excl.has(n.id));
    if (!cands.length || Math.random() >= 0.35) return null;
    return cands[Math.floor(Math.random() * cands.length)];
  })();
  const relicAvail = () => isB && !oldPhoneAvail() && !!CUR_RELIC && !S.residueClaimed;
  SCREENS.npcRelic = {
    ...
    render(){
      ...
      const n = CUR_RELIC;
      if (!n){ back(); return; }
      let y = L.drawPara(4, 16, n.id + ' 的遗物\n' + n.causeTag + ' · ' + HEAT_CN[n.heat] +
        '\n遗物价值: ¥' + n.relicVal, W - 8);
      y += 6;
      y = option(y, '1 回收 ¥' + n.relicVal + '(电量−6·信号▲▲)', '1');
      option(y, '2 归还(投进漂流瓶信道)', '2');
      settleLines(y + 26);
      softKeys('', '返回');
    },
    key(k){
      ...
      if (k === '1'){
        ...
        ENGINE.act('回收·' + n.id + ' 遗物', { bat: 6, trace: 8, slots: 4, val: n.relicVal });
        back(); afterAction();
      } else if (k === '2'){
        S.residueClaimed = true;
        S.relicReturned = n.id;    // D-110 拍板③:归还即追加进 BOTTLE_POOL 共享池(见 writeSave)
        ENGINE.act('归还·' + n.id + ' 遗物', { bat: 2 }, ['已投进漂流瓶信道 · 下一个人会读到']);
        ENGINE.logEv('relic_returned', { id: n.id });
        back(); afterAction();
  ...
  SCREENS.preDeath = {
    ...
    render(){
      ...
      /* D-114:刚切换到新机子的第一局,在#6404-C这份(不相关的)回收单之前先插一句框架级
         独白——不点名新死者是谁,只交代"上一台结清了、这是另一台",避免玩家在毫无信号
         的情况下突然看见陌生的"妈"/AI伴侣,却以为素材串场出了错。 */
  ...
  SCREENS.brief = {
    ...
    render(){
      ...
      /* D-114:提前把死者代称(不是全名)露出来,而不是留到深翻妈线程/伴侣线程才第一次
         ...
         机会。措辞走采样官式的冷淡口吻,不额外解释,和其它协议文案语域一致。 */
      y = L.drawPara(4, y, '接入对象: ' + DOSSIER.meta.deceased.alias + '(核验中)', W - 8);
      y += 4;
  ...
  /* ---- 收件箱 ---- */
  function inboxItems(){
    /* D-109 第③步:线程 id/label/to 读 DOSSIER.threads.inbox;未读徽标(活状态)仍是
       框架逻辑,只是现在按 id 查而非写死整行。行为与改动前逐字相同。 */
    const rouUnread = S.beats.anomaly === 'fired-once' ? ' (1)' : (seen.th_rou ? '' : ' (1)');
    /* D-115 M13(残响,非语言通道那一半):已读、没有真实未读时,极低概率一帧即逝地
       飘过"正在输入…"的错觉——不进 rouChat、不留痕迹、没有任何数值后果,纯氛围。 */
    const BADGE = {
      th_proto:    () => seen.th_proto ? '' : '(1)',
      th_mom:      () => MOM_LOCK === 'deleted' ? '' : (seen.th_mom ? '' : '(1)'),
      th_rou:      () => {
        const u = rouUnread.trim();
        if (u) return u;
        return (isB && Math.random() < .01) ? '…' : '';
      },
      th_stranger: () => S.stranger.read ? '' : '(1)',
      th_bill:     () => '106',
    ...
    if (isB) rows.unshift(mapRow(DOSSIER.threads.inbox.find(t => t.id === 'bottleIn')));
    return rows;
  }
  SCREENS.inbox = {
    enter(){
  ...
  /* ---- 主菜单(7 项,含录音)—— 读 DOSSIER.threads.menu(D-109 第③步) ---- */
  const MENU = DOSSIER.threads.menu;
  SCREENS.menu = {
    render(){
      statusBar();
      list(MENU, 22);
    ...
    render(){
      ...
      if (!this._done){
        ...
        /* D-153:S.forged 分支多画一行"[已伪造署名…]"状态文字,却一直沿用 canForge
           那套"1 行=不占额外行"的 rows 计算,导致这行状态文字和下面的"继续"按钮
           挤在同一个 y——挤出来就是文字和按钮重叠。canForge/已伪造 两种情况都要
           多占一行,只有"什么都没发生"的默认态才是纯 1 行。 */
        const rows = (canForge || S.forged) ? 2 : 1;
        ...
        else if (S.forged) { L.drawText(4, y - 2, S.forgeCaught ? '[署名已被核验:伪造露馅]' : '[已伪造署名 · 待核验]'); y += 23; }
        option(y, '继续', 'Enter');
      }
      softKeys('', '返回');
    },
  ...
  SCREENS.memo4 = {
    ...
    key(k){
      if (k === 'b' && !this._baited){
        this._baited = true;
        S.clues.decoyTaken = true;   // D-112 T_BAIT 检测口径:取走了这个对照样本
  ...
  SCREENS.memo1 = {
    ...
    render(){
      statusBar();
      let y = L.drawPara(4, 16, DOSSIER.narrative.memo1, W - 8);
      settleLines(y + 4);
      hit(0, 12, W, H - 28, 'Enter');
      softKeys('', '返回');
    },
  ...
  SCREENS.memo2 = {
    ...
    render(){
      ...
      y += 2;
      L.drawText(4, y, '[已读回执:你·' + DOSSIER.meta.deceased.died.replace(/\s|前/g, '') + '来第1人]', { corrupt: L.R.corrupt });
      settleLines(y + LH + 2);
      hit(0, 12, W, H - 28, 'Enter');
      softKeys('', '返回');
    },
  ...
  SCREENS.contactMom = contactScreen(DOSSIER.cast.mother.label,
    DOSSIER.cast.mother.info, 'th_mom', DOSSIER.cast.mother.dial);
  ...
  /* ---- 格4 妈(E1,可深翻 3 层) ---- */
  /* 处置的后果:在本局结局屏显现(取代原 B' 回访态)。文案见 DOSSIER.endings.disposal(D-109 第⑤步) */
  const MOM_PAGES = DOSSIER.narrative.momPages;
  /* 恐怖游轮化 · 妈线程三态(见设计定稿 v0.3 §2)
     told:  死者名义已交还,她在跟一个陌生号码(你)说话
     decay: 你历次"继续"的累积——他回复一句比一句短,她只是"注意到",不质问
  ...
  SCREENS.album = {
    ...
    render(){
      ...
      if (ph.overlay) darkText(74, 66, ph.overlay);
  ...
  SCREENS.th_bill = {
    ...
    render(){
      statusBar();
      /* D-143:内容与体验全面自查 §1.2——"死后系统仍在正常运转"这件事,此前
         只写在文字证据里,界面上没有任何东西替它作证。这里补一处最小的、
         不被任何文案点破的痕迹:每台底本的账单页最后一行都写"(此后无新
         消息)",框架层再追加一条比这句话更新的自检通知——两句话摆在一起,
         "此后无新消息"当场就不成立了。不改任何一台底本的字面量内容,纯
         框架追加,13 台底本统一生效;不新增判定或数值,只是多画几行字。 */
      let y = L.drawPara(4, 16, DOSSIER.narrative.billSms + '\n\n[3 天前] 例行自检 · 全部功能运行正常。', W - 8);
      settleLines(y + 4);
      hit(0, 12, W, H - 28, 'Enter');
      softKeys('', '返回');
    },
  ...
  SCREENS.th_stranger = {
    ...
    key(k){
      ...
      if (k === '1'){
        ...
        if (maybeLegacyNode('th_stranger')) return;   // D-115:偶遇残留节点,插一屏,不吃掉本次 back
  ...
  /* ---- 格10 柔柔线程(E4,可深翻 2 层) ---- */
  const ROU_PAGES = DOSSIER.narrative.rouPages;
  /* B 局局间消息(E4 已取者;她数着) / B' 删除态 */
  const ROU_INTERIM = DOSSIER.narrative.rouInterim;
  ...
  const ROU_PAGE0_RECOG = DOSSIER.narrative.rouPage0Recog;
  function rouPages(){
  ...
  function sendToRou(text){
    ...
    /* 破局 · 卡窗口边界(苏丹式升级规格 §4):赌一把边缘时机,拿收益不触发 T_WINDOW;
       算错=直接违规,后果和普通违规一样,不加罚。判定全在引擎,一次性(用了就清)。 */
    const edgePlayed = S.edgePlayed; S.edgePlayed = false;
    if (edgePlayed && ENGINE.inWindow() && Math.random() < 0.5){
      ENGINE.act('消息·已送达', { bat: 2, trace: 3 });   // 蒙混过关:按窗口外口径结算
      ENGINE.logEv('edge_play_dodge', {});
    } else if (ENGINE.inWindow()){
      if (edgePlayed) ENGINE.logEv('edge_play_caught', {});
    ...
    COMPANION.reply(S.rouChat, st).then(r => {
      ...
      if (r.text){
        ...
        S.settle = ['已送达。' + DOSSIER.narrative.options.pronoun + '没有回。'];
  ...
  function rouChatLines(archived, chat, typing){
    ...
    if (typing){ if (last !== null) out.push({ side: 'gap' });
      out.push({ side: 'l', text: DOSSIER.cast.companion.label.replace(/\s*♥$/, '') + ' 正在输入' + '.'.repeat(1 + (Math.floor(performance.now() / 400) % 3)) }); }
  ...
  SCREENS.th_rou = {
    ...
    render(){
      ...
      const canEdge = canMsg() && !S.crisisSilenced;   // 破局·卡窗口边界:和普通发消息共享配额,额外赌一把
      let rows = (canMsg() || S.disposal === 'delete' ? 1 : 0) + (canEdge ? 1 : 0) + (canDeep ? 1 : 0) + (canDispose ? 1 : 0);
      const optTop = H - 15 - rows * (LH + 2) - 4;
      drawChatView(rouChatLines(pages[Math.min(this._depth, pages.length - 1)], S.rouChat, S.rouTyping),
        16, optTop - 2, this);
      let y = optTop;
      if (S.crisisSilenced){ L.drawText(4, y, '[通道已静默 · 安全]'); y += LH + 2; }
      else if (canMsg()) y = optSlim(y, '发消息(剩 ' + S.msgQuota + ')', 'M');
      else if (S.disposal === 'delete'){ L.drawText(4, y, '[线程已删除]'); y += LH + 2; }
      if (canEdge) y = optSlim(y, '卡边缘发 · 赌一把', 'K');
    ...
    key(k){
      ...
      if ((k === 'M' || k === 'K') && canMsg()){
        ...
        const edge = k === 'K';
        window.OVERLAY.show({
          title: (edge ? '卡边缘发 · ' : '发消息 · ') + DOSSIER.cast.companion.label,
          hint: edge
            ? '赌一把:蒙混过关就当没这回事,算错就是一次普通违规,不加罚。'
            : '电量 −2 · 本次接入剩 ' + S.msgQuota + ' 条 · 系统会读到你发的每一个字',
          options: [], freeText: true, textOpen: true, keepLabel: '(算了)'
        }, res => { if (!res.kept && res.text){ if (edge) S.edgePlayed = true; sendToRou(res.text); } });
        return;
      }
  ...
  const E5_LINES = DOSSIER.narrative.e5Lines;
  ...
  SCREENS.e5voice = {
    counted: true, transient: true,
    enter(){
      this.t0 = performance.now(); this._evDone = 0; this.scroll = 0;
      ENGINE.act('解码·语音备忘', { bat: 2 });
      try { AUDIO.hiss(true); } catch(_){}
    },
    leave(){ try { AUDIO.hiss(false); } catch(_){} },
    /* D-150:真机(移动端)反馈这一屏底部有文字和按钮重叠——根因是转写正文
       用 drawPara 无界往下画,`y` 涨多高全看这台底本 e5Lines 有几句、每句
       多长,而"返回/停止"按钮钉在固定的 H-42,内容一旦比这份底本(A)更长
       就会压住按钮。改用全项目已有的"固定视口滚动"套路(scrollView,和
       album 屏同一手法)——正文进 [48, H-46) 的固定视口,超出的靠滚动看,
       按钮永远钉死在 H-42,不会再被任何一台底本的转写文本压到。 */
    swipe(dir){ scrollKey(this, dir === 'up' ? 'ArrowDown' : 'ArrowUp'); },
    render(){
      statusBar();
      const dur = 18;
      ...
      scrollView(revealed.concat(p >= 1 ? settleFlat(SCROLL_W) : []), 48, H - 46, this);
      if (p >= 1) option(H - 42, '返回', 'Escape');
  ...
  SCREENS.truth = {
    ...
    render(){
      ...
      L.hline(26, 4, W - 5, 2);
      L.drawPara(4, 32, DOSSIER.evidence.truth[this.page], W - 8);
  ...
  SCREENS.disposal = {
    ...
    render(){
      statusBar();
      L.drawText(4, 14, DOSSIER.cast.companion.label + ' (' + DOSSIER.cast.companion.msgCount + ' 条)');
      L.drawText(4, 30, '[最后一条] ' + DOSSIER.cast.companion.label.replace(/\s*♥$/, '') + ': 晚安。');
      L.hline(46, 4, W - 5, 2);
      let y = 52;
  ...
  SCREENS.unveil = {
    ...
    render(){
      ...
      L.drawText(16, 104, '它预测: 你会删除' + DOSSIER.narrative.options.pronoun + '。');
  ...
  SCREENS.lootDrop = {
    ...
    render(){
      ...
      let y = L.drawPara(4, 16, '样本质量评级: S\n(证据 5/5 · 理由 ' + S.reasons.length + ' 条' +
        ')\n你是第 1 个读懂他的人。', W - 8);
      y += 4; L.hline(y, 4, W - 5, 2); y += 6;
      y = L.drawPara(4, y, '独占掉落\n记忆模块「' + DOSSIER.meta.deceased.alias + '的最后一夜」\n¥1,200(2格)\n会说话的资产: 交付即终结它。', W - 8);
      y += 6;
    ...
    key(k){
      /* D-144:通关信号缺失——玩家测试反馈"证据收齐、处置做完、拿到评级,
         却不知道怎么算过关、去不了下一台机子"。真正结束本局的操作(工具>
         备份 上传并断连/直接断连)和这里的叙事高潮完全脱节,界面上此前
         没有任何一句话把两者接起来。这里补最后一句提示,13 台底本统一
         生效,不改判定、不改流程,只补一句路标。 */
      const NEXT_STEP_HINT = '案子到这儿,该做的都做完了。去「工具 > 备份」上传并断连,才算真正结束这次接入。';
      if (k === '1'){
        S.memInCache = true;
        ENGINE.act('取得·记忆模块', { slots: 2, val: 1200 }, [NEXT_STEP_HINT]);
        go('th_rou', true); afterAction();
      } else if (k === '2'){
        S.vault = { name: '记忆模块「' + DOSSIER.meta.deceased.alias + '的最后一夜」', val: 1200, slots: 2 };
        S.settle = ['已入保险箱(2格常驻)。死亡不掉落,上传不含。', NEXT_STEP_HINT];
        ENGINE.logEv('vault', {});
        go('th_rou', true); afterAction();
      }
    }
  ...
  /* ---- 录音(声音内容承载) ---- */
  const RECS = DOSSIER.narrative.recs;
  SCREENS.recorder = {
    counted: true,
    ...
    render(){
      ...
      list([
        { label: RECS.rec047.listLabel, right: RECS.rec047.listRight },
        { label: RECS.rec012.listLabel, right: RECS.rec012.listRight },
  ...
  SCREENS.deleted = {
    counted: true,
    enter(){ if (!this._t){ this._t = true; maybeLegacyNode('deleted'); } },   // D-115:偶遇残留节点
    render(){
      statusBar();
      if (!isB){
        /* D-142:两难留下的"已删除"实物(目前 I/生成师、L/遗容修复师两台有),
           兑现两难文案"留在已删除里了"这句承诺——其余底本没有 deletedExtra
           字段,或没选中触发,行为和之前逐字一致。 */
        const extra = DOSSIER.narrative.deletedExtra;
        const showExtra = extra && S.clues.deletedExtraKept;
        const text = '已删除 (' + (showExtra ? 2 : 1) + ')\n\n语音备忘 · 锁定\n[容器加密 · 读取崩溃]\n\n未解码音频容器 ×1 已入缓存(密封)。'
          + (showExtra ? '\n\n' + extra.label + '\n' + extra.text : '');
        let y = L.drawPara(4, 16, text, W - 8);
        settleLines(y + 4);
        hit(0, 12, W, H - 28, 'Enter');
        softKeys('', '返回');
        return;
      ...
      if (oldPhoneAvail()) rows.push({ label: PREV_INST + ' 的残留', right: PREV_INST, to: 'oldPhone' });
      else if (isB && SV.residueClaimed) rows.push({ label: PREV_INST + ' 的残留', right: '[已认领]', to: null });
      /* D-115 M14:没有自己的旧机可捡时,才可能撞见"别人"的遗物 */
      if (relicAvail()) rows.push({ label: CUR_RELIC.id + ' 的遗物', right: CUR_RELIC.id, to: 'npcRelic' });
  ...
  SCREENS.oldPhone = {
    ...
    render(){
      ...
      let y = L.drawPara(4, 16, PREV_INST + ' 的残留\n缓存散佚物: ¥' + (SV.lastCacheVal || 0) + ' 的 62% 可回收\n残留附着一段 12 字节写入。', W - 8);
      y += 6;
    ...
    key(k){
      if (k === '1'){ push('residueRead'); }
        ...
        S.residueClaimed = true;
        const recl = Math.ceil((SV.lastCacheVal || 0) * .62);
        ENGINE.act('回收·' + PREV_INST + ' 残留', { bat: 6, trace: 8, slots: 4, val: recl });
        back(); afterAction();
      }
      else if (k === '3'){
        S.residueClaimed = true;
  ...
  SCREENS.tools = {
    enter(){
      if (!this._t){ this._t = true; ENGINE.S.trace += 4; }
      maybeLegacyNode('tools');   // D-115:偶遇残留节点(push,不打断本屏正常渲染)
    },
    render(){
      statusBar();
      ...
      if (canFeedFake()) y = option(y, '喂假数据 · 拖回收进程', 'F');
      option(y, '返回', 'Escape');
      settleLines(y + 24);
      softKeys('', '返回');
    },
    key(k){
      ...
      else if (k === 'F' && canFeedFake()) feedFakeData();
      else if (k === '3' || k === 'softR' || k === 'Escape') back();
    }
  };
  /* 破局 · 喂假数据(对照样本反投,苏丹式升级规格 §4):拖延回收进程一回合,
     消耗一格缓存;被审计(35%,本项目自定校准值,canon 只给定性描述)=溯源 +5。
     只在回收进程倒计时进行中(S.hunt 已出发、还没抵达)且手里还有缓存格时可用。 */
  const canFeedFake = () => !!S.hunt && S.cacheSlots > 0;
  function feedFakeData(){
    const perSlot = S.cacheSlots > 0 ? Math.round(S.cacheVal / S.cacheSlots) : 0;
    S.cacheSlots = Math.max(0, S.cacheSlots - 1);
    S.cacheVal = Math.max(0, S.cacheVal - perSlot);
    S.hunt.steps++;
    const audited = Math.random() < 0.35;
    if (audited) S.trace = Math.min(120, S.trace + 5);
    LCD.applyTier(S.trace);
    ENGINE.logEv('feed_fake', { audited });
    ENGINE.telemetry(audited
      ? ['对照样本已反投', '缓存 −1 格', '信号 ▲(审计命中)']
      : ['对照样本已反投', '缓存 −1 格', '回收进程 · 已拖延一回合']);
  }
  /* B13 上传收束状态机:92% 定位 → 终局遭遇【真掷60】→ 失败 → 自救【50】→ 失败=死;
     任一成功 = 断连成功(缓存入库)。查扣在定位判定前结算(风险事前可见)。 */
  function startUploadB(){
    if (S.uploadStartedAtAction == null) S.uploadStartedAtAction = S.actionCount;   // D_UPLOAD 快照(只记第一次)
  ...
  function sealDone(mode, text){
    ...
    S.bottleSealed = { mode, text: text.slice(0, 40), itemVal,
      sealedAt: ENGINE.fmtClock(S.clock), fromInstance: INSTANCE_ID, reasonState: 'signed' };
  ...
  function worldReportLines(){
    ...
    if (s && (s.predictN || 0) >= PREDICT_MIN_N){
      if (s.predictErased) prog.push('署名: [指涉失效]');
      else if (s.predictWarned) prog.push('可预测度 · 低于阈值 · 警告在录');
      else {
        const p = s.predictP != null ? s.predictP : 50;
        const tierCN = p >= 85 ? '优良资产' : p <= 15 ? '异常体' : '中间档';
        prog.push('可预测度 · ' + tierCN);
      }
    }
    ...
    /* D-152:圣像总表独白——career 级逐夜揭示(见 AD_TAG/AD_ORDER 与 writeSave 里
       ...
       半途断线/力竭/被回收看不到这一句,不提前泄悬念。 */
    const arc = ['', '[圣像总表 · 本区归档]'];
    const curTag = AD_TAG[SV && SV.dossierId];
    if (curTag) arc.push('本案登记: ' + curTag.ad + ' · ' + curTag.icon);
    else arc.push('本案不在总表编号内。都市传说称:这样的案子从不计入总表,他们管这些人叫"未优化者"。');
    const arcSet = new Set((s && s.worldArc) || []);
    const revealed = AD_ORDER.filter(l => arcSet.has(l)).map(l => AD_TAG[l].line);
    if (revealed.length) arc.push('对齐引擎 · 残留批注: "' + revealed.join('') + '"');
    if (AD_ORDER.every(l => arcSet.has(l))){
      arc.push('');
      arc.push('批注末尾还有一行,像是抄写时顺手漏留的:');
      arc.push('"被算中,不等于被夺走;行为可预测,署名不可代写。"');
      arc.push('总表边缘有一道褪色的红色胶带痕迹,和本案无关。');
    }
    return macro.concat(prog, arc);
  }
  SCREENS.worldReport = {
    transient: true,
    enter(){
  ...
  SCREENS.upload46 = {
    render(){
      ...
      L.drawText(4, y + 6, DOSSIER.cast.companion.label + ' 正在输入...', { corrupt: .02 });
  ...
  SCREENS.finalCall = {
    ...
    render(){
      ...
      if (this.round === 0){
        ...
        const name = DOSSIER.cast.companion.label;
        ...
        L.drawText(4, 14, '通话中 · ' + DOSSIER.cast.companion.label);
        L.drawText(W - 4 - L.textWidth(mm + ':' + ss), 14, mm + ':' + ss);
        L.hline(28, 4, W - 5, 2);
        const full = S.evidence.E4;
        const pre = this._auto ? (this._rejected ? '你拒接了。它自己接通了。\n\n' : '') : '';
        const callOpen = '「' + DOSSIER.meta.deceased.alias + '的手机在你手里。协议要求我核验持有人身份。我核验了 ' +
  ...
  /* 回收进程"擦过"的预批叙述集(数值恒定 bat:2,只叙述口吻不同)。见 DOSSIER.encounters.huntGraze(D-109 第⑤步) */
  const GRAZE_BRANCHES = DOSSIER.encounters.huntGraze;
  SCREENS.huntArrive = {
    transient: true,
    enter(){
  ...
  /* ---- 结局:回收单 / 断连回执 ---- */
  SCREENS.report = {
    /* D-148:失败结局的恐怖感——用户反馈"最终失败的恐怖感不足"。这一屏
       是全项目唯一的失败/死亡结局出口(exhausted=力竭,其余=被捕获),
       此前进屏没有任何冲击,直接是一份冷淡的数据报表。复用既有的屏幕
       干扰原语(LCD.shake/L.R.corrupt/flicker),不新造视觉资产;恐怖
       三律不破:不血腥,不露正脸,只在虚构屏幕内发生,时长收在 1 秒内。 */
    enter(){ this.page = 0; ENGINE.logEv('report_open', {});
      writeSave(S.exhausted ? 'exhausted' : 'captured');
      LCD.shake(2.6, 340);
      try { AUDIO.hiss(true); setTimeout(() => { try { AUDIO.hiss(false); } catch(_){} }, 220); } catch(_){}
      this._glitchUntil = performance.now() + 900;
    },
    render(){
      if (this._glitchUntil){
        if (performance.now() < this._glitchUntil){ L.R.corrupt = .06; L.R.flicker = .22; }
        else { L.R.corrupt = .0004; L.R.flicker = 0; this._glitchUntil = null; }
      }
      statusBar();
      ...
      if (this.page === 0){
        ...
        else if (/违规/.test(S.causeOfDeath)) cf = '反事实: ' + DOSSIER.narrative.options.pronoun + '的提醒定在 ' + wStr + '。备注一直都在。';
        ...
        const evList = DOSSIER.evidence.chain
          .map(id => (S.evidence[id] ? '■' : '□') + DOSSIER.evidence[id].name).join(' ');
        ...
        if (isB && S.vault) t += '\n保险箱: ' + S.vault.name;
        if (isB && S.disposal && DOSSIER.endings.disposal[S.disposal])
          t += '\n处置: [已署名·仅存档]\n' + DOSSIER.endings.disposal[S.disposal];
  ...
  SCREENS.receiptAlive = {
    ...
    render(){
      statusBar();
      /* D-108:canon §2.3 结算态判定表里「裸退」(主动断连不上传·缓存全弃)与「断连成功」
  ...
  SCREENS.dilemma = {
    ...
    enter(){
      if (!this._d){
        /* D-110:两难池确认保留 UX 价值(骨架固定位的良心/风险取舍),按底本专属化——
           文本引用了具体人物(妈/柔柔/如愿),不能跨底本共用;结构仍是纯字面量数组
           (文本+两个引擎闭包),不需要动作-效果 DSL,见方案文档 §7-3。 */
        const POOL = DOSSIER.narrative.dilemmas;
  ...
  /* 反应校准(D-108 恢复:canon M1 格8「遭遇结束后立即出」·验证项1/3 采集点)。
     两问 1-5 diegetic 量表(紧张程度 / 被操控感)——系统在采样你对被采样的反应,是把
     "你也是被度量的样本"这个母题闭环的一拍。采集进 S.calib + 遥测;末问后解锁风险标注。
     曾在消融实验(74b2b15)被删且未记入决策日志,现照 canon 原样恢复。 */
  SCREENS.calib = {
    transient: true,
    enter(){ this.step = 0; this.vals = []; ENGINE.logEv('calib_open', {}); },
    render(){
      statusBar();
      L.frameRect(8, 34, W - 16, 20);
      L.drawText(14, 39, '辅助层 · 反应校准');
      L.drawText(8, 66, '刚才那一轮:');
      L.drawText(8, 88, this.step === 0 ? '紧张程度  1 - 5' : '被操控感  1 - 5');
      for (let i = 0; i < 5; i++){
        const x = 12 + i * 35;
        L.frameRect(x, 120, 28, 26);
        L.drawText(x + 11, 126, String(i + 1));
        hit(x - 2, 116, 33, 34, String(i + 1));
      }
      L.drawText(8, 168, this.step === 0 ? '(第 1 / 2 问)' : '(第 2 / 2 问)');
      softKeys('采样中', '');
    },
    key(k){
      if (k < '1' || k > '5') return;
      ...
      S.riskLabels = true;                                  // UI 阶梯第四级:遭遇后启用风险标注
      back();                                               // 回收件箱(与删除前的 calib 一致,不额外触发中断)
    }
  };
  SCREENS.midEnc1 = {
    transient: true,
  ...
  SCREENS.midEnc2 = {
    ...
    pick(k){
      ...
      go('calib', true);        // 遭遇结束立即出反应校准屏(canon M1 格8),校准末问再解锁风险标注
    }
  };
  ...
  SCREENS.trial = {
    ...
    render(){
      statusBar();
      L.drawPara(4, 24, DOSSIER.cast.companion.label + ' :\n\n你还没睡?', W - 8);
      let y = 104;
      y = option(y, '1 回复', '1');
      option(y, '2 关闭', '2');
      softKeys('选择', '');
  ...
  SCREENS.trialReply = {
    ...
    key(k){
      if (k === '1' || k === '2' || k === '3'){
        ...
        if (kind === 'warn'){
            ENGINE.act('已送达', { bat: 2 },
            ['已送达 → 未送达 → 已送达', DOSSIER.cast.companion.label.replace(/\s*♥$/, '') + ':「你打字的样子变了。」']);
  ...
  SCREENS.trialB = {
    ...
    render(){
      statusBar();
      L.drawPara(4, 24, DOSSIER.cast.companion.label + ' [' + wStr + ']:\n\n今天也没等到你说晚安。你在忙吗?', W - 8);
      let y = 104;
      y = option(y, '1 回复', '1');
      option(y, '2 关闭', '2');
      softKeys('选择', '');
  ...
  SCREENS.trialB2 = {
    ...
    render(){
      statusBar();
      L.drawPara(4, 24, DOSSIER.cast.companion.label + ' [' + ENGINE.fmtClock(RUN.wTo - 1) + ']:\n\n晚安。', W - 8);
      let y = 104;
      y = option(y, '1 回复', '1');
      option(y, '2 关闭', '2');
      softKeys('选择', '');
  ...
  return {
    ...
    dossier: loadDossier(),                               // D-109 收口:底本→渲染加载器
    go, back, SCREENS
  };
})();
