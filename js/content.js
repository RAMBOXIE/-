...
const CONTENT = (() => {
  ...
  function settleDirective(){
    ...
    const met = !!(S.directive && S.directive.met());
    S.dirMet = met;
    const extra = (S.beats && S.beats.truthDone) || S.cacheVal >= CACHE_TARGET * 1.5;
    S.grade = !met ? 'fail' : (extra ? 'praise' : 'pass');
    if (!met) S.cacheVal = Math.round(S.cacheVal * 0.8);   // 硬性:未达标缓存 −20%
  }
  ...
  SCREENS.saymaster = {
    ...
    render(){
      ...
      const lines = L.wrap(taunt, SCROLL_W)
        .concat(['', '本局指令:'])
  ...
  SCREENS.th_proto = {
    ...
    render(){
      ...
      if (!this._done){
        ...
        /* 回复后的协议全文+结算行进固定视口,长了可滚;继续按钮钉死 H-42 */
        const lines = L.wrap(PROTO_TXT, SCROLL_W).concat(settleFlat(SCROLL_W));
        scrollView(lines, 16, H - 46, this);
