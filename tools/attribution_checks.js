...
function driveToViolation(g){
  ...
  g.E.setClock(3, 35);                         // 进入 B 局窗口(runCount1: 03:31-03:45)
  g.C.go('trialB', true);
...
/* ---- 读 #5502-D + 窗口内违规 → 待冲洗 ---- */
scenario('置位 · 读#5502-D后窗口内违规 → pendAttr', ({A}) => {
...
scenario('冲洗 · 关闭踩坑屏弹「归因采集」卡并记录', ({A}) => {
  const g = boot(B_SAVE_5502);
  Math.random = () => 0.4;
...
scenario('回收单 · 仅作答后打印归因行(断连成功页)', ({A}) => {
  const g = boot(B_SAVE_5502);
  Math.random = () => 0.4;
  driveToViolation(g);
...
scenario('门控 · 读非#5502-D瓶后窗口内违规不置位', ({A}) => {
