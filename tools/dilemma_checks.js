...
const FILES = ['js/lcd.js','js/save.js','js/companion.js','js/grader.js','js/mom.js','js/stranger.js','js/engine.js','js/content.js'];
...
scenario('两难·替妈撒谎 · 替回涨溯源(引擎回填)', ({A, rnd}) => {
  ...
  /* D-165:底本 A 的这句结算文案在去模板化重写后换了措辞("这句谎,离病房
     又远了一寸。"),不再是"谎言又续了一分"那个具体句子——判定放宽到只
     认"谎"这个字(单字,新旧文案都含),验证的是"这个动作在叙事上被
     标成撒谎"这个机制事实,不锁死某一句具体台词。 */
  A((g.S.settle || []).join('').includes('谎'), '结算应叙及撒谎这件事,实际 ' + JSON.stringify(g.S.settle));
});
